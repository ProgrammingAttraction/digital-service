...
import { 
  ...
  FaBell, FaGift, FaCog, FaChartBar, FaFileAlt,
  FaUserShield, FaDatabase, FaHistory, FaListAlt,
  FaCreditCard, FaExchangeAlt, FaCoins, FaPercent,
  FaTrophy, FaStar, FaEnvelope, FaUserCircle,
  FaSignOutAlt, FaHeadset, FaQuestionCircle, 
  FaLock, FaUserCheck, FaUserPlus, FaUserTimes,
  FaChartPie, FaRegChartBar, FaClipboardList,
  FaRegNewspaper, FaTags, FaWallet,
  FaRegMoneyBillAlt, FaHandHoldingUsd, FaShieldAlt,
  FaCogs, FaGlobe, FaUserTag, FaUserEdit,
  FaFileInvoice, FaReceipt, FaFileExport,
  FaCertificate, FaIdCard, FaFingerprint,
  FaBox,
  FaFile, FaFileArchive, FaDollarSign, FaList,
  FaChevronDown, FaChevronUp, FaBullhorn, FaUserFriends,
  FaBars, FaBolt, FaUserPlus as FaUserPlusIcon, // Added FaUserPlus for Registration Manage
} from 'react-icons/fa';
import { MdOutlineSdCard, MdOutlineAssignmentInd } from 'react-icons/md';
...
import { 
  ...
  BsShieldCheck,
  BsGift,
  BsGlobe // Added BsGlobe as alternative
} from 'react-icons/bs';
...
import { 
  ...
  MdHistory,
  MdAppRegistration // Added for Registration icon
} from 'react-icons/md';
import { 
  FiUsers,
  FiUserCheck,
...
const Sidebar = ({ isOpen, closeSidebar }) => {
  ...
  const [openGroups, setOpenGroups] = useState([
    'Dashboard', 'User Management', 'Sub Admin Management', 'Deposit System', 
    'Orders', 'Transaction System', 'Payment Methods', 'Bonus System', 
    'Services', 'Manage List', 'Api Setup', 'Account'
  ]);
  ...
  const menuGroups = [
    {
      title: 'Dashboard',
      items: [
        { icon: <AiOutlineDashboard />, text: 'Dashboard', url: '/dashboard', id: 'dashboard' },
      ]
    },
    {
    ...
    },
    {
      ...
      items: [
        { icon: <FaUserPlusIcon />, text: 'New Sub Admin', url: '/admin/sub-admin/new', id: 'new-sub-admin' },
        { icon: <FaUserFriends />, text: 'Sub Admin List', url: '/admin/sub-admin/list', id: 'sub-admin-list' },
      ]
    },
    {
      title: 'Deposit System',
      items: [
        { icon: <GiMoneyStack />, text: 'All Deposits', url: '/admin/deposits/all', id: 'all-deposits' },
        { icon: <MdPendingActions />, text: 'Pending Deposits', url: '/admin/deposits/pending', id: 'pending-deposits' },
        { icon: <BsShieldCheck />, text: 'Approved Deposits', url: '/admin/deposits/approved', id: 'approved-deposits' },
        { icon: <RiRefund2Line />, text: 'Rejected Deposits', url: '/admin/deposits/rejected', id: 'rejected-deposits' },
      ]
    },
    {
      title: 'Orders',
    ...
    },
    {
      ...
      items: [
        { icon: <FaExchangeAlt />, text: 'All Transactions', url: '/admin/transactions/all', id: 'all-transactions' },
        { icon: <MdAccountBalanceWallet />, text: 'Balance History', url: '/admin/transactions/balance-history', id: 'balance-history' },
      ]
    },
    {
      title: 'Payment Methods',
      items: [
        { icon: <BsCreditCard />, text: 'Add Methods', url: '/admin/payment-methods/add', id: 'add-methods' },
        { icon: <FaListAlt />, text: 'All Methods', url: '/admin/payment-methods/all', id: 'all-methods' },
        { icon: <FaBolt />, text: 'Auto Method', url: '/admin/payment-methods/auto', id: 'auto-method' },
      ]
    },
    {
      title: 'Bonus System',
      items: [
        { icon: <BsGift />, text: 'Bonus Offers', url: '/admin/bonus/offers', id: 'bonus-offers' },
        { icon: <MdHistory />, text: 'Bonus History', url: '/admin/bonus/history', id: 'bonus-history' },
      ]
    },
    {
      title: 'Services',
    ...
    },
    {
      title: 'Manage List',
      items: [
        { icon: <FaBars />, text: 'Menu Manage', url: '/admin/menu-manage', id: 'menu-manage' },
        ...
        { icon: <FaGlobe />, text: 'Website Status', url: '/admin/website-status', id: 'website-status' },
        { icon: <MdAppRegistration />, text: 'Registration Page Manage', url: '/admin/registration-manage', id: 'registration-manage' }, // Added Registration Page Manage
      ]
    },
    {
      title: 'Account',
                ...
                <div className={`overflow-hidden transition-all duration-300 ${isGroupOpen ? 'max-h-[600px] opacity-100' : 'max-h-0 opacity-0'}`}>
                  <ul className="space-y-1 mt-1">
                    {group.items.map((item, iIndex) => {
                      ...
                      const activeClass = isDarkMode ? 'bg-gray-800 text-[#00a8ff] font-semibold' : ' text-[#00a8ff] font-semibold';
                      const inactiveClass = isDarkMode ? 'text-gray-300 hover:bg-gray-800 hover:text-white' : 'text-gray-600 hover:bg-gray-50';
                        ...
                        <li key={iIndex}>
                          {item.onClick ? (
                            <button 
                              ...
                              className={`w-full ${commonClass} ${inactiveClass}`}
                            >
                              <span className={`mr-3 text-lg ${isDarkMode ? 'text-gray-400' : 'text-gray-500'}`}>
                                {item.icon}
                              </span>
                              <span>{item.text}</span>
                            </button>
          ...
          <div className={`rounded-2xl shadow-2xl w-full max-w-sm overflow-hidden transform transition-all ${isDarkMode ? 'bg-gray-800' : 'bg-white'}`}>
            ...
            <div className={`px-6 py-4 flex gap-3 ${isDarkMode ? 'bg-gray-900/50' : 'bg-gray-50'}`}>
              <button 
                ...
                className={`flex-1 py-2.5 text-sm font-semibold rounded-xl border transition-all ${isDarkMode ? 'border-gray-600 text-gray-300 hover:bg-gray-700' : 'border-gray-300 text-gray-700 hover:bg-white'}`}
              >
                Cancel
              </button>
              <button 
                onClick={handleLogoutConfirm} 
                className="flex-1 py-2.5 text-sm font-semibold text-white bg-[#00a8ff] hover:opacity-90 rounded-xl shadow-lg transition-all"
              >
                Logout
              </button>
            </div>
