...
function Allorders() {
  ...
  useEffect(() => {
    ...
    if (serviceParam) {
      // Decode URL encoded service name (for Bengali/UTF-8 characters)
      const decodedService = decodeURIComponent(serviceParam);
      
      // Set the service filter immediately
      setSelectedFilters(prev => ({
        ...prev,
        service: decodedService
      }));
      
      console.log('Service filter applied from URL:', decodedService);
    }
  }, [location.search]);
  
  // Extract unique services from orders
  const extractUniqueServices = (ordersData) => {
    const services = new Set();
    ordersData.forEach(order => {
      if (order.serviceName) {
        services.add(order.serviceName);
      }
    });
  ...
  const filterAndSortOrders = (ordersData) => {
    ...
    // Apply service filter first (if exists)
    if (selectedFilters.service) {
      filteredOrders = filteredOrders.filter(order => 
        order.serviceName === selectedFilters.service
      );
    }
  ...
  const fetchOrders = useCallback(async () => {
    try {
      ...
      if (response.data.success) {
        ...
        const services = extractUniqueServices(allOrdersData);
        setAvailableServices(services);
       
        // Apply URL service filter immediately if present
        const queryParams = new URLSearchParams(location.search);
        const serviceParam = queryParams.get('service');
        ...
        if (serviceParam) {
          ...
          const serviceFilteredOrders = allOrdersData.filter(order => 
            order.serviceName === decodedService
          );
          stats = calculateStatistics(serviceFilteredOrders);
          setStatistics(stats);
          
          // Apply all filters including service filter
          filteredOrders = filterAndSortOrders(allOrdersData);
        } else {
          stats = calculateStatistics(allOrdersData);
          setStatistics(stats);
        ...
        if (response.data.totals) {
          setOrderStats(response.data.totals);
        }
      }
    } catch (error) {
  ...
  }, [base_url, token, currentPage, itemsPerPage, searchTerm, selectedFilters, location.search]);
  ...
  const handleDeleteOrder = async () => {
    ...
    try {
      setIsDeleting(true);
      const response = await axios.delete(`${base_url}/api/admin/orders/${selectedOrder._id}`, {
        headers: {
          'Authorization': `Bearer ${token}`,
          'Content-Type': 'application/json'
        }
  ...
  const handleDeleteAllOrders = async () => {
    ...
    try {
      ...
      const response = await axios.delete(`${base_url}/api/admin/orders/delete-all`, {
        headers: {
          'Authorization': `Bearer ${token}`,
          'Content-Type': 'application/json'
        },
  ...
  const handleOrderCancellation = async () => {
    ...
    try {
      ...
      const response = await axios.post(
        `${base_url}/api/admin/orders/${cancellingOrder._id}/cancel`,
        {
  ...
  const handleServiceSelect = (service) => {
    ...
    // Update URL with service filter
    if (service) {
      const encodedService = encodeURIComponent(service);
      navigate(`/admin/orders/all?service=${encodedService}`);
    } else {
  ...
  // Statistics Cards Component - Updated to show filtered statistics
  const StatisticsCards = () => {
    // Use service-filtered statistics when a service filter is active
          ...
          <div className="flex items-center justify-between">
            <div>
              <p className="text-sm text-gray-600">
                {selectedFilters.service ? 'Filtered Orders' : 'Total Orders'}
              </p>
              <p className="text-2xl font-bold text-gray-800">{displayStats.totalOrders || 0}</p>
            </div>
            <Package className="w-8 h-8 text-blue-500" />
          </div>
          <div className="mt-2 text-xs text-gray-500">
            <span className="text-green-600">Completed: {displayStats.completedOrders || 0}</span>
            {' | '}
            <span className="text-yellow-600">Pending: {displayStats.pendingOrders || 0}</span>
          </div>
        </div>
        <div className="bg-white rounded-lg shadow p-4">
          <div className="flex items-center justify-between">
            <div>
              <p className="text-sm text-gray-600">
                {selectedFilters.service ? 'Filtered Revenue' : 'Total Revenue'}
              </p>
              <p className="text-2xl font-bold text-gray-800">
                ৳{(displayStats.totalAmount || 0)?.toLocaleString()}
              </p>
            </div>
            <FaBangladeshiTakaSign className="w-8 h-8 text-green-500" />
          </div>
          <div className="mt-2 text-xs text-gray-500">
            Avg. Order: ৳{displayStats.totalOrders ? Math.round(displayStats.totalAmount / displayStats.totalOrders) : 0}
          </div>
        </div>
        <div className="bg-white rounded-lg shadow p-4">
          <div className="flex items-center justify-between">
            <div>
              ...
              <p className="text-2xl font-bold text-gray-800">
                {displayStats.processingOrders || 0}
              </p>
            </div>
            <Clock className="w-8 h-8 text-purple-500" />
          </div>
          <div className="mt-2 text-xs text-gray-500">
            Cancelled: {displayStats.cancelledOrders || 0}
          </div>
        </div>
        <div className="bg-white rounded-lg shadow p-4">
          <div className="flex items-center justify-between">
            <div>
              <p className="text-sm text-gray-600">Recent Orders (7d)</p>
              <p className="text-2xl font-bold text-gray-800">
                {displayStats.recentOrders || 0}
              </p>
            </div>
            <Calendar className="w-8 h-8 text-orange-500" />
          </div>
          <div className="mt-2 text-xs text-gray-500">
            {selectedFilters.service ? `Filtered by: ${selectedFilters.service}` : 'All Services'}
          </div>
        </div>
      </div>
    );
    ...
    <div className="font-anek text-gray-700 lg:ml-72 mt-[9vh]">
      ...
      <main className="min-h-screen bg-gray-50 p-4 md:p-6">
        <div className="w-full mx-auto">
          {/* Page Header */}
          <div className="mb-6">
            <h1 className="text-2xl font-bold text-theme_color2 mb-1">
              {selectedFilters.service ? `${selectedFilters.service} Orders` : 'All Orders'}
            </h1>
            <p className="text-gray-600 mt-1">
              {selectedFilters.service 
                ? `Showing all orders for "${selectedFilters.service}" service`
                : 'Manage all customer orders and track their status'
              }
            </p>
            
            {/* Show active service filter badge */}
            {selectedFilters.service && (
              <div className="mt-2 inline-flex items-center px-3 py-1 bg-blue-100 text-blue-800 rounded-full text-sm">
                <Filter size={14} className="mr-2" />
                Filtered by Service: <span className="font-semibold ml-1">{selectedFilters.service}</span>
                <button
              ...
              <div className="flex flex-col md:flex-row md:items-center justify-between gap-4">
                ...
                <div className="flex flex-col md:flex-row md:items-center gap-4 flex-1">
                  <div className="relative flex-1 max-w-md">
                    ...
                    <input
                      type="text"
                      placeholder={selectedFilters.service 
                        ? `Search ${selectedFilters.service} orders...`
                        : "Search by order ID, customer, service..."
                      }
                      value={searchTerm}
                  ...
                  <div className="flex flex-wrap gap-2">
                    ...
                    <div className="relative" ref={serviceDropdownRef}>
                      ...
                      >
                        ...
                        <span className="ml-2 text-sm">
                          {selectedFilters.service ? `Service: ${selectedFilters.service.substring(0, 20)}${selectedFilters.service.length > 20 ? '...' : ''}` : "Service"}
                        </span>
                        {selectedFilters.service && (
                      ...
                      {showServiceDropdown && availableServices.length > 0 && (
                        <div className="absolute top-full left-0 mt-1 w-64 cursor-pointer max-h-[300px] overflow-y-auto bg-white rounded-lg shadow-lg border border-gray-200 z-50 custom-scrollbar">
                          <div className="p-2">
                            <div className="px-3 py-2 text-xs font-medium text-gray-500 uppercase tracking-wider">
                              Filter by Service ({availableServices.length})
                            </div>
                            {availableServices.map((service) => (
                              <button
                                key={service}
                                onClick={() => handleServiceSelect(service)}
                                className={`w-full text-left px-3 text-nowrap py-2 mb-1 rounded cursor-pointer transition-colors duration-150 truncate ${
                                  selectedFilters.service === service ? 'text-theme_color2 font-medium' : 'text-gray-700'
                                }`}
                                title={service}
                              >
                                {service}
                    ...
                    {(selectedFilters.status || selectedFilters.orderType || selectedFilters.service || selectedFilters.sort !== 'newest' || searchTerm) && (
                      <button
              ...
              {loading ? (
                ...
                <div className="text-center py-12">
                  ...
                  <h3 className="text-lg font-medium text-gray-700">
                    {selectedFilters.service ? `No orders found for "${selectedFilters.service}"` : 'No orders found'}
                  </h3>
                  <p className="text-gray-500 mt-1">
                    {selectedFilters.service 
                      ? 'Try adjusting your search or other filters'
                          ...
                          <div className="flex items-center space-x-2">
                            ...
                            {order.status === 'pending' || order.status === 'processing' ? (
                              ...
                              <button
                                ...
                                className="bg-red-600 rounded-[5px] text-white cursor-pointer px-[10px] py-[8px] hover:bg-red-700 transition-colors duration-150"
                                title="Delete Order"
                              >
                                <Trash2 size={16} />
                              </button>
                    ...
                    <div className="text-sm text-gray-700">
                      ...
                      {selectedFilters.service && (
                        <span className="ml-2 text-blue-600">
                          (Filtered by: {selectedFilters.service})
                        </span>
                      )}
                    </div>
