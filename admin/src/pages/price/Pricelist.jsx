...
function Pricelist() {
  ...
  const defaultServices = [
    ...
    { name: "জন্মনিবন্ধন ডাটা", price: "0", isActive: true },
    ...
    { name: "নাম্বার টু লোকেশন", price: "0", isActive: true },
    { name: "IMEI TO NUMBAR", price: "0", isActive: true },
    { name: "এনআইডি ইউজার পাস", price: "0", isActive: true },
    { name: "নতুন জন্ম নিবন্ধন", price: "0", isActive: true },
    { name: "টিন সার্টিফিকেট অর্ডার", price: "0", isActive: true },
    { name: "জিরো রিটার্ন", price: "0", isActive: true },
    { name: "এনআইডি টু অল নাম্বার", price: "0", isActive: true },
    { name: "নাম ঠিকানা 2 এন আইডি", price: "0", isActive: true },
    { name: "স্মার্ট কার্ড অর্ডার", price: "0", isActive: true },
    { name: "কার্ড অর্ডার", price: "0", isActive: true },
    { name: "সার্ভার কপি অর্ডার", price: "0", isActive: true }
  ];
  ...
  const fetchPriceList = async () => {
    try {
      setLoading(true);
      const response = await axios.get(`${base_url}/api/admin/pricelist`, {
        headers: {
  ...
  const handleSavePrice = async (id) => {
    ...
    try {
      ...
      const response = await axios.patch(
        `${base_url}/api/admin/pricelist/${id}/price`,
        { price: priceValue },
        {
          headers: {
            'Authorization': `Bearer ${token}`,
    ...
    try {
      ...
      if (existingServices.length > 0) {
        ...
        const response = await axios.put(
          `${base_url}/api/admin/pricelist/bulk/price`,
          { priceUpdates },
          {
            headers: {
              'Authorization': `Bearer ${token}`,
  ...
  const handleBulkDelete = async () => {
    ...
    if (apiServicesToDelete.length > 0) {
      try {
        setLoading(true);
        const response = await axios.post(
          `${base_url}/api/admin/pricelist/bulk-delete`,
  ...
  const handleToggleStatus = async (id) => {
    ...
    try {
      setLoading(true);
      const response = await axios.patch(
        `${base_url}/api/admin/pricelist/${id}/toggle-status`,
        {},
        {
          headers: {
            'Authorization': `Bearer ${token}`
                    ...
                    {filteredServices.map((service) => (
                      <tr key={service.id} className="hover:bg-gray-50 transition-colors duration-150">
                        ...
                        <td className="px-6 py-4 whitespace-nowrap">
                          ...
                          >
                            <span className={`w-2 h-2 rounded-full mr-2 ${service.isActive ? 'bg-green-500' : 'bg-red-500'}`}></span>
