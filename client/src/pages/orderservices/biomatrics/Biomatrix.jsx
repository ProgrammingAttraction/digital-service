...
function Biomatrix() {
  ...
  const [selectedSim, setSelectedSim] = useState('bl');
  ...
  // Function to detect SIM operator from phone number
  const detectSimOperator = (number) => {
    if (number.length < 3) return null;
    
    const prefix = number.substring(0, 3);
    
    for (const [operator, prefixes] of Object.entries(simPrefixes)) {
  ...
  const fetchPrices = async () => {
    try {
      ...
      const response = await axios.get(`${base_url}/api/user/service/price/biometrix-order`);
      console.log('Price API response:', response.data);
      
      // Update prices based on API response
      setPrices({
        bl: response.data.price1 || 100,
        robi: response.data.price2 || 150,
        teletalk: response.data.price4 || 250,
        gp: response.data.price5 || 120,
        airtel: response.data.price6 || 130,
        birthNumber: response.data.price3 || 300
      });
    } catch (error) {
  ...
  const fetchNotice = async () => {
    try {
      ...
      const response = await axios.get(`${base_url}/api/user/service/notice/biomatrix`);
      if (response.data) {
  ...
  // Fetch orders for biometrix service
  const fetchOrders = async () => {
    try {
      ...
      if (response.data.success) {
        // Filter orders by service name containing "বায়োমেট্রিক" or "জন্ম নম্বর"
        const biometrixOrders = response.data.data.filter(order => 
          (order.serviceName && (order.serviceName.includes('বায়োমেট্রিক') || order.serviceName.includes('জন্ম নম্বর'))) ||
          (order.serviceType && (order.serviceType.includes('biomatrix') || order.serviceType.includes('birth_number')))
        );
        setOrders(biometrixOrders);
      }
    } catch (error) {
  ...
  const handlePhoneNumberChange = (e) => {
    ...
    // Auto-detect SIM operator when phone number is valid
    if (numericValue.length >= 3) {
      const detectedOperator = detectSimOperator(numericValue);
      if (detectedOperator && simOptions.find(sim => sim.id === detectedOperator)) {
        setSelectedSim(detectedOperator);
        
        // Show toast notification for auto-selection
        const selectedSimData = simOptions.find(sim => sim.id === detectedOperator);
        if (selectedSimData) {
          toast.success(`${selectedSimData.name} স্বয়ংক্রিয়ভাবে নির্বাচিত হয়েছে`, {
            duration: 2000,
            position: 'top-center',
          });
        }
  ...
  // SIM card options with logos and dynamic prices
  const simOptions = [
    { 
      id: 'robi', 
      name: 'Robi',
      shortName: 'Robi',
      logo: 'https://images.seeklogo.com/logo-png/45/2/robi-logo-png_seeklogo-454716.png',
      price: prices.robi,
      prefixes: ['016', '018']
    },
    { 
      ...
      name: 'Banglalink',
      shortName: 'BL',
      logo: 'https://static.vecteezy.com/system/resources/thumbnails/068/842/057/small_2x/banglalink-logo-bl-sim-company-icon-transparent-background-free-png.png',
      price: prices.bl,
      prefixes: ['014', '019']
    },
    { 
      ...
      name: 'Teletalk',
      shortName: 'Teletalk',
      logo: 'https://upload.wikimedia.org/wikipedia/en/thumb/3/3f/Teletalk_Bangladesh_Limited.svg/1200px-Teletalk_Bangladesh_Limited.svg.png',
      price: prices.teletalk,
      prefixes: ['015']
    },
    { 
      id: 'airtel', 
      name: 'Airtel',
      shortName: 'Airtel',
      logo: 'https://play-lh.googleusercontent.com/uFg3zOsnGZkIrswmvXyFYhoF3gC4tv0ovFZv0zisJFQ2DZqJyh9SUGrK6D-Tnn1lGqc=w600-h300-pc0xffffff-pd',
      price: prices.airtel,
      prefixes: ['011']
    },
  ];

  // Find selected SIM data
  const selectedSimData = simOptions.find(sim => sim.id === selectedSim);
  ...
  const validateForm = () => {
    ...
    if (selectedSim !== 'birthNumber') {
      ...
      // Validate if the SIM operator matches the phone number prefix
      const detectedOperator = detectSimOperator(phoneNumber);
      if (detectedOperator !== selectedSim) {
        const selectedSimInfo = simOptions.find(sim => sim.id === selectedSim);
        const detectedSimInfo = simOptions.find(sim => sim.id === detectedOperator);
        
        if (detectedSimInfo) {
          return `এই ফোন নম্বরটি ${detectedSimInfo.name} এর। দয়া করে ${detectedSimInfo.name} সিলেক্ট করুন অথবা সঠিক ফোন নম্বর দিন।`;
        }
      }
    } else {
      // Birth Number validation
      if (phoneNumber.length < 10) {
        return 'জন্ম নম্বর ভুল হয়েছে';
      }
    }
  ...
  // Handle SIM selection manually
  const handleSimSelect = (simId) => {
    setSelectedSim(simId);
    
    // If user manually selects a SIM, validate current phone number
    if (phoneNumber.length >= 3 && simId !== 'birthNumber') {
      const detectedOperator = detectSimOperator(phoneNumber);
      if (detectedOperator && detectedOperator !== simId) {
        const selectedSimInfo = simOptions.find(sim => sim.id === simId);
        const detectedSimInfo = simOptions.find(sim => sim.id === detectedOperator);
        
        toast.warning(`এই ফোন নম্বরটি ${detectedSimInfo.name} এর। আপনি ${selectedSimInfo.name} নির্বাচন করেছেন।`, {
          duration: 3000,
          position: 'top-center',
        });
      }
  ...
  const handleSubmit = async () => {
    ...
    try {
      ...
      const requestData = {
        serviceId: `biomatrix_${selectedSim}`,
        serviceName: selectedSim === 'birthNumber' 
          ? 'জন্ম নম্বর' 
          : `বায়োমেট্রিক (${selectedSimData.name})`,
        serviceRate: selectedSimData.price,
        serviceType: selectedSim === 'birthNumber' ? 'birth_number' : 'biomatrix',
        orderType: 'text_file',
        quantity: 1,
        notes: selectedSim === 'birthNumber'
          ? `জন্ম নম্বর: ${phoneNumber}`
          : `ফোন নম্বর: ${phoneNumber}\nসিম অপারেটর: ${selectedSimData.name}`,
        fieldValues: {
          phoneNumber: phoneNumber,
          simOperator: selectedSimData.name,
          simShortName: selectedSimData.shortName
        },
        totalAmount: selectedSimData.price,
        urgency: 'normal'
      };
      ...
      if (response.data.success) {
        ...
        // Reset form
        setPhoneNumber('');
        setSelectedSim('bl'); // Reset to default
  ...
  // Get current phone number's SIM operator (for display)
  const getCurrentPhoneSimInfo = () => {
    if (phoneNumber.length >= 3 && selectedSim !== 'birthNumber') {
      const detectedOperator = detectSimOperator(phoneNumber);
      if (detectedOperator) {
        return simOptions.find(sim => sim.id === detectedOperator);
      }
    }
    return null;
  };

  const currentPhoneSimInfo = getCurrentPhoneSimInfo();
  const isSimMismatch = currentPhoneSimInfo && currentPhoneSimInfo.id !== selectedSim;
          ...
          <div className={`rounded-2xl p-6 md:p-8 max-w-md w-full shadow-xl transition-colors duration-300 ${
            ...
            <div className="text-center">
              ...
              <p className={`text-sm md:text-base mb-4 md:mb-6 transition-colors duration-300 ${
                ...
                {selectedSim === 'birthNumber' 
                  ? 'আপনার জন্ম নম্বর অর্ডারটি সফলভাবে প্লেস করা হয়েছে।'
                  : 'আপনার বায়োমেট্রিক অর্ডারটি সফলভাবে প্লেস করা হয়েছে।'}
              </p>
                ...
                <div className="grid grid-cols-2 gap-3 md:gap-4">
                  ...
                  <div className="text-left">
                    <p className={`text-xs md:text-sm transition-colors duration-300 ${
                      ...
                      {selectedSim === 'birthNumber' ? 'জন্ম নম্বর' : 'ফোন নম্বর'}
                    </p>
          ...
          <div className={`p-4 md:p-6 w-full border rounded-lg shadow-sm transition-colors duration-300 ${
            ...
            <div className="mb-5">
              <h1 className={`text-xl md:text-2xl font-bold transition-colors duration-300 ${
                ...
                {selectedSim === 'birthNumber' ? 'জন্ম নম্বর অর্ডার' : 'বায়োমেট্রিক অর্ডার'}
              </h1>
            </div>
            ...
            <div className="mb-6">
              <div className="flex items-center justify-between mb-3">
                <label className={`text-sm md:text-base font-medium transition-colors duration-300 ${
                  isDarkMode ? 'text-gray-400' : 'text-gray-700'
                }`}>
                  {selectedSim === 'birthNumber' ? 'সেবা নির্বাচন করুন *' : 'সিম অপারেটর নির্বাচন করুন *'}
                </label>
                
                {/* Auto-detection status */}
                {phoneNumber.length >= 3 && currentPhoneSimInfo && !isSimMismatch && (
                  <span className="text-xs bg-green-500 text-white px-2 py-1 rounded-full">
                    স্বয়ংক্রিয় নির্বাচিত
                  </span>
                )}
              </div>
                ...
                {simOptions.map((sim) => (
                  <div 
                    ...
                    className={`cursor-pointer rounded-lg overflow-hidden border-2 transition-all duration-300 ${
                      selectedSim === sim.id 
                        ? isSimMismatch && currentPhoneSimInfo?.id === sim.id
                          ? isDarkMode
                            ? 'border-yellow-500 shadow-lg shadow-yellow-500/20'
                            : 'border-yellow-400 shadow-lg shadow-yellow-400/20'
                          : isDarkMode 
                            ? 'border-green-500 shadow-md' 
                            : 'border-theme_color2 shadow-md'
                        : currentPhoneSimInfo?.id === sim.id
                          ? isDarkMode
                  ...
                  >
                    <div className={`p-4 transition-colors duration-300 ${
                      ...
                      <div className="text-center mt-2">
                        ...
                        <div className={`text-base font-bold mt-1 transition-colors duration-300 ${
                          isDarkMode 
                            ? selectedSim === sim.id 
                              ? 'text-green-400' 
                              : currentPhoneSimInfo?.id === sim.id
                        ...
                        {/* Prefix indicator */}
                        {sim.prefixes && sim.prefixes.length > 0 && (
                ...
                <div className={`mt-4 p-3 rounded-lg border flex items-start gap-3 transition-colors duration-300 ${
                  ...
                  <div>
                    <p className="font-medium text-yellow-700 dark:text-yellow-300">
                      সিম অপারেটর মিলছে না!
                    </p>
                    <p className="text-sm text-yellow-600 dark:text-yellow-400">
                      আপনার ফোন নম্বরটি <strong>{currentPhoneSimInfo.name}</strong> এর, কিন্তু আপনি <strong>{selectedSimData.name}</strong> নির্বাচন করেছেন। দয়া করে {currentPhoneSimInfo.name} নির্বাচন করুন অথবা সঠিক ফোন নম্বর দিন।
                    </p>
                    <button
                      onClick={() => handleSimSelect(currentPhoneSimInfo.id)}
                      className="mt-2 text-sm bg-yellow-500 hover:bg-yellow-600 text-white px-3 py-1 rounded-md transition duration-200"
                    >
                      {currentPhoneSimInfo.name} সিলেক্ট করুন
                    </button>
                  </div>
                </div>
              )}
            ...
            <div className="mb-6">
              <label className={`text-sm md:text-base font-medium mb-2 block transition-colors duration-300 ${
                isDarkMode ? 'text-gray-400' : 'text-gray-700'
              }`}>
                {selectedSim === 'birthNumber' ? 'জন্ম নম্বর *' : 'ফোন নম্বর *'}
              </label>
              <div className="flex flex-col md:flex-row gap-3">
                <div className="flex-1">
                  <input 
                    ...
                    placeholder={selectedSim === 'birthNumber' ? 'জন্ম নম্বর লিখুন' : "01XXXXXXXXX (11 ডিজিট)"}
                    ...
                    maxLength={selectedSim === 'birthNumber' ? "20" : "11"}
                  />
                </div>
              </div>
              
              {/* SIM Detection Indicator */}
              {phoneNumber.length >= 3 && currentPhoneSimInfo && selectedSim !== 'birthNumber' && (
                <div className={`mt-2 text-sm flex items-center gap-2 ${
                  isSimMismatch ? 'text-yellow-500' : 'text-green-500'
                }`}>
                  {isSimMismatch ? (
                    <>
                      ...
                      <span>
                        এই নম্বরটি <strong>{currentPhoneSimInfo.name}</strong> এর, কিন্তু আপনি <strong>{selectedSimData.name}</strong> নির্বাচন করেছেন।
                      </span>
                    </>
                  ) : (
                    <>
                      ...
                      <span>
                        স্বয়ংক্রিয়ভাবে সনাক্তকৃত: <strong>{currentPhoneSimInfo.name}</strong>
                      </span>
                    </>
                  )}
                </div>
            ...
            {submitError && !submitError.includes('আবশ্যক') && !submitError.includes('ডিজিট') && !submitError.includes('জন্ম') && (
            ...
            <div className={`mt-6 mb-4 p-3 text-base border rounded-md flex justify-start items-center gap-2 transition-colors duration-300 ${
              ...
              <p className={isDarkMode ? 'text-blue-300' : 'text-blue-700'}>
                এই সেবার জন্য আপনার অ্যাকাউন্ট থেকে 
                <span className="font-bold mx-1">
                  {loadingPrices ? '...' : selectedSimData?.price} 
                </span>
                টাকা কাটা হবে। 
              </p>
            </div>
            ...
            <div className="mt-6">
              <button 
                ...
                disabled={loading || loadingPrices || (isSimMismatch && selectedSim !== 'birthNumber')}
                className={`w-full text-white font-bold py-2.5 rounded-md transition-colors text-[15px] flex items-center justify-center ${
                  loading || loadingPrices
                    ...
                    : isSimMismatch && selectedSim !== 'birthNumber'
                    ? isDarkMode
              ...
              >
                {loading ? (
                  ...
                  `অর্ডার করুন (${selectedSimData?.price} TK)`
                )}
              </button>
            </div>
          </div>
                        ...
                        <td colSpan="7" className="p-8 text-center">
                          <div className="flex flex-col items-center justify-center">
                            ...
                            <p className={`text-sm max-w-md transition-colors duration-300 ${
                              ...
                              {searchTerm ? 'আপনার সার্চের সাথে মিলছে এমন কোনো অর্ডার নেই' : 'আপনার প্রথম বায়োমেট্রিক অর্ডারটি এখনই করুন!'}
                            </p>
                            {searchTerm && (
                              <button
