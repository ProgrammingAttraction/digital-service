...
function Servercopyorder() {
  ...
  const fetchNotice = async () => {
    try {
      ...
      const response = await axios.get(`${base_url}/api/user/service/notice/server-copy-order`);
      if (response.data) {
        setNotice(response.data.service);
      } else {
        setNotice('⚠️ নোটিশঃ সার্ভার কপি সার্ভিস। এনআইডি নাম্বার এবং জন্ম তারিখ অবশ্যই সঠিক দিতে হবে।');
      }
    } catch (error) {
  ...
  const fetchPrices = async () => {
    try {
      ...
      const response = await axios.get(`${base_url}/api/user/service/price/server-copy-order`);
      // Update service prices based on backend response
      const updatedPrices = {
        serverCopy: response.data.price
      };
  ...
  // Fetch orders for server copy service
  const fetchOrders = async () => {
    try {
      ...
      if (response.data.success) {
        // Filter orders by service name containing "সার্ভার কপি"
        const serverCopyOrders = response.data.data.filter(order => 
          order.serviceName && order.serviceName.includes('সার্ভার কপি')
        );
        setOrders(serverCopyOrders);
      }
    } catch (error) {
  ...
  const validateForm = () => {
    ...
    if (!birthDate.trim()) {
      errors.birthDate = 'জন্ম তারিখ আবশ্যক';
  ...
  const handleSubmit = async (e) => {
    ...
    try {
      ...
      let dynamicData = `এনআইডি নাম্বার - ${nidNumber}\nজন্ম তারিখ - ${birthDate}`;
      if (extraInfo.trim()) {
        dynamicData += `\nঅতিরিক্ত তথ্য - ${extraInfo}`;
      }
      ...
      const requestData = {
        serviceId: 'server_copy',
        serviceName: 'সার্ভার কপি',
        serviceRate: servicePrices.serverCopy,
        serviceType: 'server_copy',
        orderType: 'pdf_file',
        quantity: 1,
        notes: dynamicData,
        fieldValues: fieldValues,
        totalAmount: servicePrices.serverCopy,
        urgency: 'normal'
      };
            ...
            <form onSubmit={handleSubmit}>
              <div className="space-y-4">
                ...
                <div>
                  ...
                  <p className={`mt-1 text-xs ${isDarkMode ? 'text-gray-400' : 'text-gray-500'}`}>
                    সঠিক জন্ম তারিখ (যেমনঃ 1990-05-15)
                  </p>
                </div>
                ...
                <div>
                  ...
                  <textarea 
                    ...
                    placeholder="যেকোনো অতিরিক্ত তথ্য লিখুন (যেমনঃ পূর্বের কোন সমস্যা, বিশেষ নির্দেশনা ইত্যাদি)" 
                    rows="3"
                  ...
                  />
                  <p className={`mt-1 text-xs ${isDarkMode ? 'text-gray-400' : 'text-gray-500'}`}>
                    সার্ভার কপি সম্পর্কিত যেকোনো অতিরিক্ত তথ্য লিখতে পারেন
                  </p>
                </div>
              </div>
              ...
              <div className={`mt-4 mb-4 p-3 text-base border rounded-md flex justify-start items-center gap-2 transition-colors duration-300 ${
                ...
                <p className={isDarkMode ? 'text-blue-300' : 'text-blue-700'}>
                  এই সেবার জন্য আপনার অ্যাকাউন্ট থেকে <span className="font-bold">{pricesLoading ? '...' : servicePrices.serverCopy} টাকা</span> কাটা হবে। 
                </p>
              </div>
              ...
              <div className="mt-6">
                ...
                >
                  {loading ? (
                    ...
                    `অর্ডার করুন (${pricesLoading ? '...' : servicePrices.serverCopy} TK)`
                  )}
                </button>
              </div>
            </form>
                        ...
                        <td colSpan="7" className="p-8 text-center">
                          <div className="flex flex-col items-center justify-center">
                            ...
                            <p className={`text-sm max-w-md transition-colors duration-300 ${
                              ...
                              {searchTerm ? 'আপনার সার্চের সাথে মিলছে এমন কোনো অর্ডার নেই' : 'আপনার প্রথম সার্ভার কপি অর্ডারটি এখনই করুন!'}
                            </p>
                            {searchTerm && (
                              <button
...
export default Servercopyorder;