...
function NidUserPass() {
  ...
  const fetchNotice = async () => {
    try {
      ...
      const response = await axios.get(`${base_url}/api/user/service/notice/nid-user-pass`);
      if (response.data) {
        setNotice(response.data.service);
      } else {
        setNotice('⚠️ নোটিশঃ এনআইডি নাম্বার দিয়ে ইউজার আইডি ও পাসওয়ার্ড সার্ভিস। এনআইডি নাম্বারটি অবশ্যই সঠিক দিতে হবে।');
      }
    } catch (error) {
  ...
  const fetchPrices = async () => {
    try {
      ...
      const response = await axios.get(`${base_url}/api/user/service/price/nid-user-to-pass`);
      // Update service prices based on backend response
      const updatedPrices = {
        nidUserPass: response.data.price
      };
  ...
  // Fetch orders for NID User Pass service
  const fetchOrders = async () => {
    try {
      ...
      if (response.data.success) {
        // Filter orders by service name containing "এনআইডি ইউজার পাস"
        const nidUserPassOrders = response.data.data.filter(order => 
          order.serviceName && order.serviceName.includes('এনআইডি ইউজার পাস')
        );
        setOrders(nidUserPassOrders);
      }
    } catch (error) {
  ...
  const handleSubmit = async (e) => {
    ...
    try {
      ...
      const requestData = {
        serviceId: 'nid_user_pass',
        serviceName: 'এনআইডি ইউজার পাস',
        serviceRate: servicePrices.nidUserPass,
        serviceType: 'nid_user_pass',
        orderType: 'text_file',
        quantity: 1,
        notes: dynamicData,
        fieldValues: fieldValues,
        totalAmount: servicePrices.nidUserPass,
        urgency: 'normal'
      };
            ...
            <form onSubmit={handleSubmit}>
              ...
              {/* Price Information */}
                ...
                <p className={isDarkMode ? 'text-blue-300' : 'text-blue-700'}>
                  এই সেবার জন্য আপনার অ্যাকাউন্ট থেকে <span className="font-bold">{pricesLoading ? '...' : servicePrices.nidUserPass} টাকা</span> কাটা হবে। 
                </p>
              </div>
              ...
              <div className="mt-6">
                ...
                >
                  {loading ? (
                    ...
                    `অর্ডার করুন (${pricesLoading ? '...' : servicePrices.nidUserPass} TK)`
                  )}
                </button>
              </div>
            </form>
          ...
          <div className={`mt-8 border rounded-lg shadow-sm p-4 md:p-6 transition-colors duration-300 ${
            ...
            <div className="flex justify-between items-center mb-6">
              <h2 className={`text-xl font-bold transition-colors duration-300 ${
                ...
                এনআইডি ইউজার পাস অর্ডার ইতিহাস
              </h2>
                        ...
                        <td colSpan="7" className="p-8 text-center">
                          <div className="flex flex-col items-center justify-center">
                            ...
                            <p className={`text-sm max-w-md transition-colors duration-300 ${
                              ...
                              {searchTerm ? 'আপনার সার্চের সাথে মিলছে এমন কোনো অর্ডার নেই' : 'আপনার প্রথম এনআইডি ইউজার পাস অর্ডারটি এখনই করুন!'}
                            </p>
                            {searchTerm && (
                              <button
