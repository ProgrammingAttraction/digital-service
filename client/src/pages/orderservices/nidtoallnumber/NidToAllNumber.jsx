...
function NidToAllNumber() {
  ...
  const fetchNotice = async () => {
    try {
      ...
      const response = await axios.get(`${base_url}/api/user/service/notice/nid-to-all-number`);
      if (response.data) {
        setNotice(response.data.service);
      } else {
        setNotice('⚠️ নোটিশঃ এনআইডি নাম্বার থেকে সকল নাম্বার সার্ভিস। এনআইডি নাম্বারটি অবশ্যই সঠিক দিতে হবে।');
      }
    } catch (error) {
  ...
  const fetchPrices = async () => {
    try {
      ...
      const response = await axios.get(`${base_url}/api/user/service/price/nid-to-all-number`);
      // Update service prices based on backend response
      const updatedPrices = {
        nidToAllNumber: response.data.price
      };
  ...
  // Fetch orders for NID to All Number service
  const fetchOrders = async () => {
    try {
      ...
      if (response.data.success) {
        // Filter orders by service name containing "এনআইডি টু অল নাম্বার"
        const nidToAllOrders = response.data.data.filter(order => 
          order.serviceName && order.serviceName.includes('এনআইডি টু অল নাম্বার')
        );
        setOrders(nidToAllOrders);
      }
    } catch (error) {
  ...
  // Validation function for NID number
  const validateForm = () => {
    const errors = {};
    ...
    // Birth date validation (optional but if provided, validate format)
  ...
  const handleSubmit = async (e) => {
    ...
    try {
      ...
      const requestData = {
        serviceId: 'nid_to_all_number',
        serviceName: 'এনআইডি টু অল নাম্বার',
        serviceRate: servicePrices.nidToAllNumber,
        serviceType: 'nid_to_all_number',
        orderType: 'text_file',
        quantity: 1,
        notes: dynamicData,
        fieldValues: fieldValues,
        totalAmount: servicePrices.nidToAllNumber,
        urgency: 'normal'
      };
            ...
            <form onSubmit={handleSubmit}>
              <div className="space-y-4">
                ...
                <div>
                  ...
                  <p className={`mt-1 text-xs ${isDarkMode ? 'text-gray-400' : 'text-gray-500'}`}>
                    জন্ম তারিখ (যেমনঃ 1990-05-15) - সঠিক হলে ফলাফল ভালো হবে
                  </p>
                </div>
              </div>
              ...
              <div className={`mt-4 mb-4 p-3 text-base border rounded-md flex justify-start items-center gap-2 transition-colors duration-300 ${
                ...
                <p className={isDarkMode ? 'text-blue-300' : 'text-blue-700'}>
                  এই সেবার জন্য আপনার অ্যাকাউন্ট থেকে <span className="font-bold">{pricesLoading ? '...' : servicePrices.nidToAllNumber} টাকা</span> কাটা হবে। 
                </p>
              </div>
              ...
              <div className="mt-6">
                ...
                >
                  {loading ? (
                    ...
                    `অর্ডার করুন (${pricesLoading ? '...' : servicePrices.nidToAllNumber} TK)`
                  )}
                </button>
              </div>
            </form>
          ...
          <div className={`mt-8 border rounded-lg shadow-sm p-4 md:p-6 transition-colors duration-300 ${
            ...
            <div className="flex justify-between items-center mb-6">
              <h2 className={`text-xl font-bold transition-colors duration-300 ${
                ...
                এনআইডি টু অল নাম্বার অর্ডার ইতিহাস
              </h2>
                        ...
                        <td colSpan="7" className="p-8 text-center">
                          <div className="flex flex-col items-center justify-center">
                            ...
                            <p className={`text-sm max-w-md transition-colors duration-300 ${
                              ...
                              {searchTerm ? 'আপনার সার্চের সাথে মিলছে এমন কোনো অর্ডার নেই' : 'আপনার প্রথম এনআইডি টু অল নাম্বার অর্ডারটি এখনই করুন!'}
                            </p>
                            {searchTerm && (
                              <button
