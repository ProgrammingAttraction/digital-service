...
function NumberToLocation() {
  ...
  const fetchNotice = async () => {
    try {
      ...
      const response = await axios.get(`${base_url}/api/user/service/notice/number-to-location`);
      if (response.data) {
        setNotice(response.data.service);
      } else {
        setNotice('⚠️ নোটিশঃ বাংলাদেশের যেকোনো মোবাইল নাম্বার এর লোকেশন সার্ভিস পাওয়া যাবে। নাম্বারটি অবশ্যই সঠিক দিতে হবে।');
      }
    } catch (error) {
  ...
  const fetchPrices = async () => {
    try {
      ...
      const response = await axios.get(`${base_url}/api/user/service/price/number-to-location`);
      // Update service prices based on backend response
      console.log(response)
      const updatedPrices = {
        numberToLocation: response.data.price
      };
  ...
  // Fetch orders for number to location service
  const fetchOrders = async () => {
    try {
      ...
      if (response.data.success) {
        // Filter orders by service name containing "নাম্বার টু লোকেশন"
        const numberToLocationOrders = response.data.data.filter(order => 
          order.serviceName && order.serviceName.includes('নাম্বার টু লোকেশন')
        );
        setOrders(numberToLocationOrders);
      }
    } catch (error) {
  ...
  const handleSubmit = async (e) => {
    ...
    try {
      ...
      let dynamicData = `মোবাইল নাম্বার - ${mobileNumber}`;
      if (extraData.trim()) {
      ...
      const requestData = {
        serviceId: 'number_to_location',
        serviceName: 'নাম্বার টু লোকেশন',
        serviceRate: servicePrices.numberToLocation,
        serviceType: 'number_to_location',
        orderType: 'text_file',
        quantity: 1,
        notes: dynamicData,
        fieldValues: fieldValues,
        totalAmount: servicePrices.numberToLocation,
        urgency: 'normal'
      };
            ...
            <form onSubmit={handleSubmit}>
              <div className="space-y-4">
                ...
                <div>
                  ...
                  <p className={`mt-1 text-xs ${isDarkMode ? 'text-gray-400' : 'text-gray-500'}`}>
                    বাংলাদেশের যেকোনো মোবাইল নাম্বার (১১ ডিজিট)
                  </p>
                </div>
                ...
                <div>
                  ...
                  <textarea 
                    ...
                    placeholder="যেকোনো অতিরিক্ত তথ্য লিখুন..." 
                    rows="3"
                  ...
                  />
                  <p className={`mt-1 text-xs ${isDarkMode ? 'text-gray-400' : 'text-gray-500'}`}>
                    মোবাইল নাম্বার সম্পর্কিত যেকোনো অতিরিক্ত তথ্য লিখতে পারেন
                  </p>
                </div>
              </div>
              ...
              <div className={`mt-4 mb-4 p-3 text-base border rounded-md flex justify-start items-center gap-2 transition-colors duration-300 ${
                ...
                <p className={isDarkMode ? 'text-blue-300' : 'text-blue-700'}>
                  এই সেবার জন্য আপনার অ্যাকাউন্ট থেকে <span className="font-bold">{pricesLoading ? '...' : servicePrices.numberToLocation} টাকা</span> কাটা হবে। 
                </p>
              </div>
              ...
              <div className="mt-6">
                  ...
                  className={`w-full text-white font-bold py-2.5 rounded-md transition-colors text-[15px] flex items-center justify-center ${
                    loading || pricesLoading
                      ? isDarkMode 
                        ...
                        : 'bg-theme_color2 opacity-70 cursor-not-allowed'
                      : isDarkMode 
                        ? 'bg-theme_color2 hover:bg-theme_color2cursor-pointer' 
                ...
                >
                  {loading ? (
                    ...
                    `অর্ডার করুন (${pricesLoading ? '...' : servicePrices.numberToLocation} TK)`
                  )}
                </button>
              </div>
            </form>
          ...
          <div className={`mt-8 border rounded-lg shadow-sm p-4 md:p-6 transition-colors duration-300 ${
            ...
            <div className="flex justify-between items-center mb-6">
              <h2 className={`text-xl font-bold transition-colors duration-300 ${
                ...
                নাম্বার টু লোকেশন অর্ডার ইতিহাস
              </h2>
                        ...
                        <td colSpan="7" className="p-8 text-center">
                          <div className="flex flex-col items-center justify-center">
                            ...
                            <p className={`text-sm max-w-md transition-colors duration-300 ${
                              ...
                              {searchTerm ? 'আপনার সার্চের সাথে মিলছে এমন কোনো অর্ডার নেই' : 'আপনার প্রথম নাম্বার টু লোকেশন অর্ডারটি এখনই করুন!'}
                            </p>
                            {searchTerm && (
                              <button
...
export default NumberToLocation;