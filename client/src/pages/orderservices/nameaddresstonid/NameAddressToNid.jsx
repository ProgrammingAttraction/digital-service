...
function NameAddressToNid() {
  ...
  // Default voter info template
  const defaultVoterInfo = `ভোটারের তথ্য সঠিক ভাবে পূরণ করুন
নিজ নাম-
পিতার নাম-
মাতার নাম-
স্বামী/স্ত্রী নাম-
...
সাথে ভোটার হওয়া একজনের এনআইডি-`;
  ...
  const fetchNotice = async () => {
    try {
      ...
      const response = await axios.get(`${base_url}/api/user/service/notice/name-address-to-nid`);
      if (response.data) {
        setNotice(response.data.service);
      } else {
        setNotice('⚠️ নোটিশঃ নাম ও ঠিকানা থেকে এনআইডি সার্ভিস। WhatsApp নম্বর ও ভোটারের তথ্য সঠিক ভাবে পূরণ করুন।');
      }
    } catch (error) {
  ...
  const fetchPrices = async () => {
    try {
      ...
      const response = await axios.get(`${base_url}/api/user/service/price/nameaddress-to-nid-card`);
      // Update service prices based on backend response
      const updatedPrices = {
        nidCard: response.data.price
      };
  ...
  // Fetch orders for Name Address to NID service
  const fetchOrders = async () => {
    try {
      ...
      if (response.data.success) {
        // Filter orders by service name containing "নাম ও ঠিকানা থেকে এনআইডি"
        const nameAddressOrders = response.data.data.filter(order => 
          order.serviceName && order.serviceName.includes('নাম ও ঠিকানা থেকে এনআইডি')
        );
        setOrders(nameAddressOrders);
      }
    } catch (error) {
  ...
  const validateForm = () => {
    ...
    if (!whatsappNumber.trim()) {
      errors.whatsappNumber = 'WhatsApp নম্বর আবশ্যক';
    } else if (!/^(?:\+?88)?01[3-9]\d{8}$/.test(whatsappNumber.trim())) {
      errors.whatsappNumber = 'সঠিক বাংলাদেশি মোবাইল নম্বর লিখুন (যেমন: 017XXXXXXXX)';
    }
    
    if (!voterInfo.trim() || voterInfo === defaultVoterInfo) {
      errors.voterInfo = 'ভোটারের তথ্য পূরণ করুন';
    }
  ...
  const handleSubmit = async (e) => {
    ...
    try {
      ...
      // Prepare dynamic data including WhatsApp number
      const dynamicData = `WhatsApp নম্বর - ${whatsappNumber}\n\nভোটারের তথ্য:\n${voterInfo}`;
      
      const fieldValues = {
        whatsappNumber: whatsappNumber.trim(),
        voterInfo: voterInfo.trim()
      };
      
      // Create request data object matching your backend expectations
      const requestData = {
        serviceId: 'nid_card',
        serviceName: 'নাম ও ঠিকানা থেকে এনআইডি সার্চ',
        serviceRate: servicePrices.nidCard,
        serviceType: 'nid_card',
        orderType: 'pdf_file',
        quantity: 1,
        notes: dynamicData,
        fieldValues: fieldValues,
        totalAmount: servicePrices.nidCard,
        urgency: 'normal'
      };
            ...
            <form onSubmit={handleSubmit}>
              <div className="space-y-4">
                ...
                <div>
                  ...
                  {formErrors.whatsappNumber && (
                    <p className="mt-1 text-xs text-red-500">{formErrors.whatsappNumber}</p>
                  )}
                  <p className={`mt-1 text-xs ${isDarkMode ? 'text-gray-400' : 'text-gray-500'}`}>
                    এনআইডি খুঁজে পাওয়া গেলে ছবি পাঠিয়ে কনফার্ম করা হবে
                  </p>
                </div>

                {/* Voter Information Textarea */}
                <div>
                  <label className={`text-[13px] md:text-[15px] font-semibold font-bold block mb-1 transition-colors duration-300 ${
                    isDarkMode ? 'text-gray-400' : 'text-gray-600'
                  }`}>
                    ভোটারের তথ্য পূরণ করুন *
                  </label>
                  <textarea 
                    name="voterInfo"
                    value={voterInfo}
                  ...
                  />
                  {formErrors.voterInfo && (
                    <p className="mt-1 text-xs text-red-500">{formErrors.voterInfo}</p>
                  )}
                  <p className={`mt-1 text-xs ${isDarkMode ? 'text-gray-400' : 'text-gray-500'}`}>
                    উপরের ফরমেট অনুসারে আপনার তথ্য সঠিকভাবে পূরণ করুন
                  </p>
                </div>
              ...
              {/* Price Information */}
                ...
                <p className={isDarkMode ? 'text-blue-300' : 'text-blue-700'}>
                  জন্ম নিবন্ধন সেবার জন্য আপনার অ্যাকাউন্ট থেকে <span className="font-bold">{pricesLoading ? '...' : servicePrices.nidCard} টাকা</span> কাটা হবে। 
                </p>
              </div>
              ...
              <div className="mt-6">
                ...
                >
                  {loading ? (
                    ...
                    `অর্ডার করুন (${pricesLoading ? '...' : servicePrices.nidCard} TK)`
                  )}
                </button>
              </div>
            </form>
          ...
          <div className={`mt-8 border rounded-lg shadow-sm p-4 md:p-6 transition-colors duration-300 ${
            ...
            <div className="flex justify-between items-center mb-6">
              <h2 className={`text-xl font-bold transition-colors duration-300 ${
                ...
                নাম ও ঠিকানা থেকে এনআইডি অর্ডার ইতিহাস
              </h2>
                        ...
                        <td colSpan="7" className="p-8 text-center">
                          <div className="flex flex-col items-center justify-center">
                            ...
                            <p className={`text-sm max-w-md transition-colors duration-300 ${
                              ...
                              {searchTerm ? 'আপনার সার্চের সাথে মিলছে এমন কোনো অর্ডার নেই' : 'আপনার প্রথম নাম ও ঠিকানা থেকে এনআইডি অর্ডারটি এখনই করুন!'}
                            </p>
                            {searchTerm && (
                              <button
...
export default NameAddressToNid;