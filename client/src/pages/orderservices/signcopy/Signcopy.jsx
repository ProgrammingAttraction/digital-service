...
import google_forms_img from "../../../assets/google-forms.png"
import id_cards_img from "../../../assets/id-card.png"
import birth_img from "../../../assets/birth.png"
import voter_img from "../../../assets/voter.png"
import { 
  Search, 
  ChevronLeft, 
  ChevronRight, 
...
function Signcopy() {
  ...
  const fetchNotice = async () => {
    try {
      ...
      const response = await axios.get(`${base_url}/api/user/service/notice/sign-copy-order`);
      if (response.data) {
  ...
  const fetchPrices = async () => {
    try {
      ...
      const response = await axios.get(`${base_url}/api/user/service/price/sign-copy-order`);
      // Update service prices based on backend response
      const updatedPrices = {
        form: response.data.price1 || 0,
        nid: response.data.price2 || 0,
        birth: response.data.price3 || 0,
        voter: response.data.price4 || 0
      };
  ...
  // Fetch orders for sign copy service
  const fetchOrders = async () => {
    try {
      ...
      if (response.data.success) {
        // Filter orders by service name containing "সাইন কপি"
        const signCopyOrders = response.data.data.filter(order => 
          order.serviceName && order.serviceName.includes('সাইন কপি')
        );
        setOrders(signCopyOrders);
      }
    } catch (error) {
  ...
  // Data for the 4 type cards with dynamic prices
  const types = [
    { 
      id: 'form', 
      img: google_forms_img, 
      label: 'FORM No', 
      price: servicePrices.form,
      fieldLabel: 'ফরম নাম্বারঃ *'
    },
    { 
      id: 'nid', 
      img: id_cards_img, 
      label: 'NID No', 
      price: servicePrices.nid,
      fieldLabel: 'এনআইডি নাম্বারঃ *'
    },
    { 
      id: 'birth', 
      img: birth_img, 
      label: 'Birth Number', 
      price: servicePrices.birth,
      fieldLabel: 'জন্মনিবন্ধন নাম্বারঃ *'
    },
    { 
      id: 'voter', 
      img: voter_img, 
      label: 'Voter Number', 
      price: servicePrices.voter,
      fieldLabel: 'ভোটার নাম্বারঃ *'
    },
  ];

  // Find selected type to display price and field label
  const selectedTypeData = types.find(type => type.id === selectedType);

  // Validation function - Birth date is now optional for all types
  const validateForm = () => {
    const errors = {};
    ...
    if (!formData.number.trim()) {
      errors.number = `${selectedTypeData.label} নাম্বার আবশ্যক`;
    }
    
    // Birth date is now optional for all types, so no validation needed
  ...
  const handleSubmit = async (e) => {
    ...
    try {
      ...
      // Prepare dynamic data based on selected type
      let dynamicData = '';
      let fieldValues = {};
      
      if (selectedType === 'form') {
        dynamicData = `নাম - ${formData.name}\nফরম নাম্বার - ${formData.number}`;
        if (formData.birthDate) {
          dynamicData += `\nজন্ম তারিখ - ${formData.birthDate}`;
        }
        fieldValues = {
          name: formData.name,
      ...
      } else if (selectedType === 'nid') {
        dynamicData = `নাম - ${formData.name}\nএনআইডি নাম্বার - ${formData.number}`;
        if (formData.birthDate) {
          dynamicData += `\nজন্ম তারিখ - ${formData.birthDate}`;
        }
        fieldValues = {
          name: formData.name,
          nidNumber: formData.number,
          ...(formData.birthDate && { birthDate: formData.birthDate })
        };
      } else if (selectedType === 'birth') {
        dynamicData = `নাম - ${formData.name}\nজন্মনিবন্ধন নাম্বার - ${formData.number}`;
        if (formData.birthDate) {
          dynamicData += `\nজন্ম তারিখ - ${formData.birthDate}`;
        }
        fieldValues = {
          name: formData.name,
          birthCertificateNumber: formData.number,
          ...(formData.birthDate && { birthDate: formData.birthDate })
        };
      } else if (selectedType === 'voter') {
        dynamicData = `নাম - ${formData.name}\nভোটার নাম্বার - ${formData.number}`;
        if (formData.birthDate) {
          dynamicData += `\nজন্ম তারিখ - ${formData.birthDate}`;
        }
        fieldValues = {
          name: formData.name,
          voterNumber: formData.number,
          ...(formData.birthDate && { birthDate: formData.birthDate })
        };
      }
      ...
      const requestData = {
        serviceId: `sign_copy_${selectedType}`,
        serviceName: `সাইন কপি (${selectedTypeData.label})`,
        serviceRate: selectedTypeData.price,
        serviceType: 'sign_copy',
        orderType: 'pdf_file',
        quantity: 1,
        notes: dynamicData,
        fieldValues: fieldValues,
        totalAmount: selectedTypeData.price,
        urgency: 'normal'
      };
      ...
      <main className='p-4 md:p-6'>
        <div className="mx-auto">
          ...
          <div className={`p-4 md:p-6 w-full border rounded-lg shadow-sm transition-colors duration-300 ${
            ...
            <div className="mb-5">
              <label className={`text-[13px] md:text-[15px] font-semibold mb-2 block transition-colors duration-300 ${
                isDarkMode ? 'text-gray-400' : 'text-gray-600'
              }`}>
                Type *
              </label>
              <div className="grid grid-cols-2 md:grid-cols-4 gap-3">
                {types.map((type) => (
                  <div 
                    ...
                    className={`cursor-pointer rounded-md py-[10px] overflow-hidden border transition-all duration-300 ${
                      selectedType === type.id 
                        ? isDarkMode 
                          ? 'border-green-500 shadow-md bg-gray-700 scale-[1.02]' 
                          : 'border-theme_color2 shadow-sm scale-[1.02]'
                        : isDarkMode 
                  ...
                  >
                    ...
                    <img 
                      src={type.img} 
                      alt={type.label} 
                      className="w-20 block m-auto h-20"
                    />
                    <div className={`py-1.5 px-1 text-center transition-colors duration-300 ${
                      isDarkMode 
                        ? selectedType === type.id ? '' : 'bg-gray-800'
                        : 'bg-white'
                    }`}>
                      <span className={`text-base font-bold block transition-colors duration-300 ${
                        isDarkMode 
                          ? selectedType === type.id ? 'text-gray-100' : 'text-gray-300'
                          : 'text-gray-500'
                      }`}>
                        {type.label}
                      </span>
                   <span className={`text-[13px] md:text-[15px] font-semibold mt-0.5 transition-colors duration-300 ${
                          isDarkMode 
                            ? selectedType === type.id ? 'text-gray-800' : 'text-gray-500'
                            : 'text-gray-400'
                        }`}>
                          ({type.price} TK)
                        </span>
            ...
            <form onSubmit={handleSubmit}>
              <div className="space-y-4">
                ...
                {/* Number Field with dynamic label based on selected type */}
                <div>
                  <label className={`text-[13px] md:text-[15px] font-semibold font-bold block mb-1 transition-colors duration-300 ${
                    isDarkMode ? 'text-gray-400' : 'text-gray-600'
                  }`}>
                    {selectedTypeData?.fieldLabel || 'এনআইডি/ভোটার/জন্মনিবন্ধন/ফরম নাম্বারঃ *'}
                  </label>
                  <input 
                    type="text"
                    name="number"
                    ...
                    onChange={handleInputChange}
                    placeholder={selectedType === 'form' ? 'ফরম নাম্বার' : 
                                selectedType === 'nid' ? 'এনআইডি নাম্বার' : 
                                selectedType === 'birth' ? 'জন্মনিবন্ধন নাম্বার' : 'ভোটার নাম্বার'} 
                    className={`w-full border rounded-md p-2 text-sm md:text-[15px] focus:outline-none focus:ring-1 h-[45px] placeholder:text-gray-400 transition-colors duration-300 ${
                      formErrors.number 
                        ? isDarkMode ? 'border-red-500 focus:ring-red-500' : 'border-red-400 focus:ring-red-400'
                        : isDarkMode 
                ...
                {/* Birth Date Field - Now optional for ALL types */}
                <div>
                  <label className={`text-[13px] md:text-[15px] font-semibold font-bold block mb-1 transition-colors duration-300 ${
                    isDarkMode ? 'text-gray-400' : 'text-gray-600'
                  }`}>
              ...
              <div className={`mt-6 mb-4 p-3 text-base border rounded-md flex justify-start items-center gap-2 transition-colors duration-300 ${
                ...
                <p className={isDarkMode ? 'text-blue-300' : 'text-blue-700'}>
                  এই সেবার জন্য আপনার অ্যাকাউন্ট থেকে <span className="font-bold">{pricesLoading ? '...' : selectedTypeData?.price || 25} টাকা</span> কাটা হবে। 
                </p>
              </div>
              ...
              <div className="mt-6">
                ...
                >
                  {loading ? (
                    ...
                    `অর্ডার করুন (${pricesLoading ? '...' : selectedTypeData?.price || 25} TK)`
                  )}
                </button>
              </div>
            </form>
                        ...
                        <td colSpan="7" className="p-8 text-center">
                          <div className="flex flex-col items-center justify-center">
                            ...
                            <p className={`text-sm max-w-md transition-colors duration-300 ${
                              ...
                              {searchTerm ? 'আপনার সার্চের সাথে মিলছে এমন কোনো অর্ডার নেই' : 'আপনার প্রথম সাইন কপি অর্ডারটি এখনই করুন!'}
                            </p>
                            {searchTerm && (
                              <button
