...
function NewBirthCertificate() {
  ...
  const fetchNotice = async () => {
    try {
      ...
      const response = await axios.get(`${base_url}/api/user/service/notice/new-death-certificate`);
      if (response.data) {
        setNotice(response.data.service);
      } else {
        setNotice('⚠️ নোটিশঃ জন্ম নিবন্ধনের জন্য সকল তথ্য সঠিকভাবে প্রদান করুন। ভুল তথ্য দিলে অর্ডার বাতিল করা হতে পারে।');
      }
    } catch (error) {
  ...
  const fetchPrices = async () => {
    try {
      ...
      const response = await axios.get(`${base_url}/api/user/service/price/new-birth-certificate`);
      // Update service prices based on backend response
      const updatedPrices = {
        birthCertificate: response.data.price
      };
  ...
  // Fetch orders for birth certificate service
  const fetchOrders = async () => {
    try {
      ...
      if (response.data.success) {
        // Filter orders by service name containing "জন্ম নিবন্ধন"
        const birthCertificateOrders = response.data.data.filter(order => 
          order.serviceName && order.serviceName.includes('জন্ম নিবন্ধন')
        );
        setOrders(birthCertificateOrders);
      }
    } catch (error) {
  ...
  const validateForm = () => {
    ...
    const requiredFields = [
      'nameBengali', 'nameEnglish', 'gender', 'mobile', 'dateOfBirth',
      'placeOfBirth', 'location', 'subdivision', 'unionTerritory',
      'address', 'postOffice', 'village', 'fatherName', 'motherName'
    ];
    
    requiredFields.forEach(field => {
      if (!formData[field]?.trim()) {
        errors[field] = 'এই তথ্যটি আবশ্যক';
      }
    });
    
    // Mobile number validation
    if (formData.mobile && !/^01[3-9]\d{8}$/.test(formData.mobile.trim())) {
      errors.mobile = 'সঠিক ১১ ডিজিটের মোবাইল নাম্বার লিখুন';
    }
    ...
    if (formData.dateOfBirth) {
      ...
      if (!dateRegex.test(formData.dateOfBirth)) {
        errors.dateOfBirth = 'ফরম্যাট: mm/dd/yyyy';
      }
    }
  ...
  const handleSubmit = async (e) => {
    ...
    if (Object.keys(errors).length > 0) {
      ...
      toast.error('কিছু তথ্য ভুলভাবে দেওয়া হয়েছে। অনুগ্রহ করে চেক করুন।', {
        duration: 3000,
        position: 'top-center',
      });
      return;
    ...
    try {
      ...
      // Prepare dynamic data for notes
      let dynamicData = `জন্ম নিবন্ধন তথ্য\n`;
      dynamicData += `নাম (বাংলা): ${formData.nameBengali}\n`;
      dynamicData += `নাম (ইংরেজি): ${formData.nameEnglish}\n`;
      dynamicData += `লিঙ্গ: ${formData.gender}\n`;
      dynamicData += `মোবাইল: ${formData.mobile}\n`;
      dynamicData += `জন্ম তারিখ: ${formData.dateOfBirth}\n`;
      dynamicData += `জন্ম স্থান: ${formData.placeOfBirth}\n`;
      dynamicData += `পিতার নাম: ${formData.fatherName}\n`;
      dynamicData += `মাতার নাম: ${formData.motherName}`;
      
      // Create request data object matching your backend expectations
      const requestData = {
        serviceId: 'birth_certificate',
        serviceName: 'জন্ম নিবন্ধন',
        serviceRate: servicePrices.birthCertificate,
        serviceType: 'birth_certificate',
        orderType: 'pdf_file',
        quantity: 1,
        notes: dynamicData,
        fieldValues: formData,
        totalAmount: servicePrices.birthCertificate,
        urgency: 'normal'
      };
      
      console.log('Sending birth certificate order data:', requestData);
          ...
          <div className={`rounded-2xl p-6 md:p-8 max-w-md w-full shadow-xl transition-colors duration-300 ${
            ...
            <div className="text-center">
              ...
              <p className={`text-sm md:text-base mb-4 md:mb-6 transition-colors duration-300 ${
                ...
                আপনার জন্ম নিবন্ধন অর্ডারটি সফলভাবে প্লেস করা হয়েছে।
              </p>
                  ...
                  <div className={`p-4 rounded-lg border transition-colors duration-300 ${
                    ...
                    <div className="space-y-3">
                      ...
                      <div className={`p-3 rounded border transition-colors duration-300 ${
                        ...
                        <h6 className={`font-bold mb-2 flex items-center gap-1 transition-colors duration-300 ${
                          isDarkMode ? 'text-gray-300' : 'text-gray-700'
                        }`}>
                          <Baby size={14} /> শিশুর তথ্য
                        </h6>
                        <div className="grid grid-cols-1 md:grid-cols-2 gap-2 text-sm">
                          {selectedOrder.fieldValues.nameBengali && (
                            <div className="flex justify-between">
                              <span className={`transition-colors duration-300 ${
                                isDarkMode ? 'text-gray-400' : 'text-gray-600'
                              }`}>
                              ...
                              <span className="font-medium">{selectedOrder.fieldValues.nameBengali}</span>
                            </div>
                          )}
                          {selectedOrder.fieldValues.nameEnglish && (
                            <div className="flex justify-between">
                              <span className={`transition-colors duration-300 ${
                                isDarkMode ? 'text-gray-400' : 'text-gray-600'
                              }`}>
                              ...
                              <span className="font-medium">{selectedOrder.fieldValues.nameEnglish}</span>
                            </div>
                          )}
                          {selectedOrder.fieldValues.gender && (
                            <div className="flex justify-between">
                              <span className={`transition-colors duration-300 ${
                                isDarkMode ? 'text-gray-400' : 'text-gray-600'
                              }`}>
                              ...
                              <span className="font-medium">{selectedOrder.fieldValues.gender}</span>
                            </div>
                          )}
                          {selectedOrder.fieldValues.dateOfBirth && (
                            <div className="flex justify-between">
                              <span className={`transition-colors duration-300 ${
                                isDarkMode ? 'text-gray-400' : 'text-gray-600'
                              }`}>
                              ...
                              <span className="font-medium">{selectedOrder.fieldValues.dateOfBirth}</span>
                            </div>
                          )}
                          {selectedOrder.fieldValues.placeOfBirth && (
                            <div className="flex justify-between">
                              <span className={`transition-colors duration-300 ${
                                isDarkMode ? 'text-gray-400' : 'text-gray-600'
                              }`}>
                              ...
                              <span className="font-medium">{selectedOrder.fieldValues.placeOfBirth}</span>
                            </div>
                          )}
                          {selectedOrder.fieldValues.childNumber && (
                            <div className="flex justify-between">
                              <span className={`transition-colors duration-300 ${
                                isDarkMode ? 'text-gray-400' : 'text-gray-600'
                              }`}>
                              ...
                              <span className="font-medium">{selectedOrder.fieldValues.childNumber}</span>
                            </div>
                          )}
                          {selectedOrder.fieldValues.mobile && (
                            <div className="flex justify-between">
                              <span className={`transition-colors duration-300 ${
                                isDarkMode ? 'text-gray-400' : 'text-gray-600'
                              }`}>
                              ...
                              <span className="font-medium">{selectedOrder.fieldValues.mobile}</span>
                            </div>
                          )}
                        </div>
                      </div>
                      ...
                      <div className={`p-3 rounded border transition-colors duration-300 ${
                        ...
                        <div className="grid grid-cols-1 md:grid-cols-2 gap-2 text-sm">
                          {selectedOrder.fieldValues.location && (
                            <div className="flex justify-between">
                              <span className={`transition-colors duration-300 ${
                                isDarkMode ? 'text-gray-400' : 'text-gray-600'
                              }`}>
                              ...
                              <span className="font-medium">{selectedOrder.fieldValues.location}</span>
                            </div>
                          )}
                          {selectedOrder.fieldValues.subdivision && (
                            <div className="flex justify-between">
                              <span className={`transition-colors duration-300 ${
                                isDarkMode ? 'text-gray-400' : 'text-gray-600'
                              }`}>
                              ...
                              <span className="font-medium">{selectedOrder.fieldValues.subdivision}</span>
                            </div>
                          )}
                          {selectedOrder.fieldValues.unionTerritory && (
                            <div className="flex justify-between">
                              <span className={`transition-colors duration-300 ${
                                isDarkMode ? 'text-gray-400' : 'text-gray-600'
                              }`}>
                              ...
                              <span className="font-medium">{selectedOrder.fieldValues.unionTerritory}</span>
                            </div>
                          )}
                          {selectedOrder.fieldValues.village && (
                            <div className="flex justify-between">
                              <span className={`transition-colors duration-300 ${
                                isDarkMode ? 'text-gray-400' : 'text-gray-600'
                              }`}>
                              ...
                              <span className="font-medium">{selectedOrder.fieldValues.village}</span>
                            </div>
                          )}
                          {selectedOrder.fieldValues.postOffice && (
                            <div className="flex justify-between">
                              <span className={`transition-colors duration-300 ${
                                isDarkMode ? 'text-gray-400' : 'text-gray-600'
                              }`}>
                              ...
                              <span className="font-medium">{selectedOrder.fieldValues.postOffice}</span>
                            </div>
                          )}
                          {selectedOrder.fieldValues.address && (
                            <div className="flex justify-between">
                              <span className={`transition-colors duration-300 ${
                                isDarkMode ? 'text-gray-400' : 'text-gray-600'
                              }`}>
                              ...
                              <span className="font-medium">{selectedOrder.fieldValues.address}</span>
                            </div>
                          )}
                        </div>
                      </div>
                      ...
                      <div className={`p-3 rounded border transition-colors duration-300 ${
                        ...
                        <h6 className={`font-bold mb-2 flex items-center gap-1 transition-colors duration-300 ${
                          ...
                          <Users size={14} /> পিতা-মাতার তথ্য
                        </h6>
                        <div className="grid grid-cols-1 md:grid-cols-2 gap-2 text-sm">
                          {selectedOrder.fieldValues.fatherName && (
                            <div className="flex justify-between">
                              <span className={`transition-colors duration-300 ${
                                isDarkMode ? 'text-gray-400' : 'text-gray-600'
                              }`}>
                              ...
                              <span className="font-medium">{selectedOrder.fieldValues.fatherName}</span>
                            </div>
                          )}
                          {selectedOrder.fieldValues.motherName && (
                            <div className="flex justify-between">
                              <span className={`transition-colors duration-300 ${
                                isDarkMode ? 'text-gray-400' : 'text-gray-600'
                              }`}>
                              ...
                              <span className="font-medium">{selectedOrder.fieldValues.motherName}</span>
                            </div>
                          )}
                        </div>
                      </div>
            ...
            <form onSubmit={handleSubmit}>
              <div className="space-y-6">
                ...
                <div className={`p-4 rounded-lg border transition-colors duration-300 ${
                  ...
                  <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
                    {/* Name in Bengali */}
                    <div>
                      <label className={`text-[13px] md:text-[15px] font-semibold block mb-1 transition-colors duration-300 ${
                        isDarkMode ? 'text-gray-400' : 'text-gray-600'
                      }`}>
                        নাম (বাংলা) *
                      </label>
                      ...
                      />
                      {formErrors.nameBengali && (
                        <p className="mt-1 text-xs text-red-500">{formErrors.nameBengali}</p>
                      )}
                    </div>
                    ...
                    <div>
                      ...
                      />
                      {formErrors.nameEnglish && (
                        <p className="mt-1 text-xs text-red-500">{formErrors.nameEnglish}</p>
                      )}
                    </div>
                    ...
                    <div>
                      ...
                      />
                      {formErrors.placeOfBirth && (
                        <p className="mt-1 text-xs text-red-500">{formErrors.placeOfBirth}</p>
                      )}
                    </div>
                    ...
                    <div>
                      ...
                      <input 
                        ...
                        onChange={handleInputChange}
                        placeholder="পরিবারের অবস্থা লিখুন" 
                        className={`w-full border rounded-md p-2 text-sm md:text-[15px] focus:outline-none focus:ring-1 h-[45px] placeholder:text-gray-400 transition-colors duration-300 ${
                          isDarkMode 
                            ? 'border-gray-600 bg-gray-700 text-gray-100 focus:ring-green-500 focus:border-green-500' 
                            : 'border-gray-300 focus:ring-theme_color2 focus:border-theme_color2'
                ...
                <div className={`p-4 rounded-lg border transition-colors duration-300 ${
                  ...
                  <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
                    ...
                    <div>
                      ...
                      />
                      {formErrors.subdivision && (
                        <p className="mt-1 text-xs text-red-500">{formErrors.subdivision}</p>
                      )}
                    </div>
                    ...
                    <div>
                      ...
                      <input 
                        type="text"
                        name="unionTerritory"
                        value={formData.unionTerritory}
                        onChange={handleInputChange}
                        placeholder="ইউনিয়ন বা পৌরসভার নাম" 
                        className={`w-full border rounded-md p-2 text-sm md:text-[15px] focus:outline-none focus:ring-1 h-[45px] placeholder:text-gray-400 transition-colors duration-300 ${
                          formErrors.unionTerritory 
                            ? isDarkMode ? 'border-red-500 focus:ring-red-500' : 'border-red-400 focus:ring-red-400'
                            : isDarkMode 
                              ? 'border-gray-600 bg-gray-700 text-gray-100 focus:ring-green-500 focus:border-green-500' 
                              : 'border-gray-300 focus:ring-theme_color2 focus:border-theme_color2'
                      ...
                      />
                      {formErrors.unionTerritory && (
                        <p className="mt-1 text-xs text-red-500">{formErrors.unionTerritory}</p>
                      )}
                    </div>
                    ...
                    <div>
                      ...
                      />
                      {formErrors.postOffice && (
                        <p className="mt-1 text-xs text-red-500">{formErrors.postOffice}</p>
                      )}
                    </div>
                    ...
                    <div>
                      ...
                      <input 
                        type="text"
                        name="postOfficeEnglish"
                        value={formData.postOfficeEnglish}
                        onChange={handleInputChange}
                    ...
                    <div>
                      ...
                      />
                      {formErrors.village && (
                        <p className="mt-1 text-xs text-red-500">{formErrors.village}</p>
                      )}
                    </div>
                    ...
                    <div>
                      ...
                      <input 
                        ...
                        onChange={handleInputChange}
                        placeholder="বাসা বা হোল্ডিং নম্বর" 
                        className={`w-full border rounded-md p-2 text-sm md:text-[15px] focus:outline-none focus:ring-1 h-[45px] placeholder:text-gray-400 transition-colors duration-300 ${
                          isDarkMode 
                            ? 'border-gray-600 bg-gray-700 text-gray-100 focus:ring-green-500 focus:border-green-500' 
                            : 'border-gray-300 focus:ring-theme_color2 focus:border-theme_color2'
                ...
                <div className={`p-4 rounded-lg border transition-colors duration-300 ${
                  ...
                  <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
                    ...
                    <div>
                      ...
                      />
                      {formErrors.fatherName && (
                        <p className="mt-1 text-xs text-red-500">{formErrors.fatherName}</p>
                      )}
                    </div>

                    {/* Father's Birth Number */}
                    <div>
                      <label className={`text-[13px] md:text-[15px] font-semibold block mb-1 transition-colors duration-300 ${
                        isDarkMode ? 'text-gray-400' : 'text-gray-600'
                      }`}>
                        জন্ম নিবন্ধন নং (পিতা)
                      </label>
                      <input 
                        type="text"
                        name="fatherBirthNumber"
                        value={formData.fatherBirthNumber}
                        onChange={handleInputChange}
                ...
                <div className={`p-4 rounded-lg border transition-colors duration-300 ${
                  ...
                  <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
                    ...
                    {/* Mother's Birth Number */}
                    <div>
                      <label className={`text-[13px] md:text-[15px] font-semibold block mb-1 transition-colors duration-300 ${
                        isDarkMode ? 'text-gray-400' : 'text-gray-600'
                      }`}>
                        জন্ম নিবন্ধন নং (মাতা)
                      </label>
                      <input 
                        type="text"
                        name="motherBirthNumber"
                        value={formData.motherBirthNumber}
                        onChange={handleInputChange}
                        placeholder="জন্ম নিবন্ধন নম্বর" 
                        className={`w-full border rounded-md p-2 text-sm md:text-[15px] focus:outline-none focus:ring-1 h-[45px] placeholder:text-gray-400 transition-colors duration-300 ${
                          isDarkMode 
              ...
              <div className={`mt-4 mb-4 p-3 text-base border rounded-md flex justify-start items-center gap-2 transition-colors duration-300 ${
                ...
                <p className={isDarkMode ? 'text-blue-300' : 'text-blue-700'}>
                  জন্ম নিবন্ধন সেবার জন্য আপনার অ্যাকাউন্ট থেকে <span className="font-bold">{pricesLoading ? '...' : servicePrices.birthCertificate} টাকা</span> কাটা হবে। 
                </p>
              </div>
              ...
              <div className="mt-6">
                ...
                >
                  {loading ? (
                    ...
                    `জন্ম নিবন্ধন অর্ডার করুন (${pricesLoading ? '...' : servicePrices.birthCertificate} TK)`
                  )}
                </button>
              </div>
            </form>
          ...
          <div className={`mt-8 border rounded-lg shadow-sm p-4 md:p-6 transition-colors duration-300 ${
            ...
            <div className="flex flex-col md:flex-row justify-between items-start md:items-center mb-6 gap-4">
              ...
              <div className={`text-sm transition-colors duration-300 ${
                ...
                মোট অর্ডার: <span className={`font-bold transition-colors duration-300 ${
                  isDarkMode ? 'text-gray-300' : 'text-gray-800'
                }`}>
                        ...
                        <td colSpan="7" className="p-8 text-center">
                          <div className="flex flex-col items-center justify-center">
                            ...
                            <p className={`text-sm max-w-md transition-colors duration-300 ${
                              ...
                              {searchTerm ? 'আপনার সার্চের সাথে মিলছে এমন কোনো অর্ডার নেই' : 'আপনার প্রথম জন্ম নিবন্ধন অর্ডারটি এখনই করুন!'}
                            </p>
                            {searchTerm && (
                              <button
