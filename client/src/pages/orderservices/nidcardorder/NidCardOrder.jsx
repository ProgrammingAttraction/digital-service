...
function NidCardOrder() {
  ...
  const fetchNotice = async () => {
    try {
      ...
      const response = await axios.get(`${base_url}/api/user/service/notice/nid-card-order`);
      if (response.data) {
        setNotice(response.data.service);
      } else {
        setNotice('⚠️ নোটিশঃ এনআইডি কার্ড অর্ডার সার্ভিস। এনআইডি নাম্বার এবং নাম অবশ্যই সঠিক দিতে হবে।');
      }
    } catch (error) {
  ...
  const fetchPrices = async () => {
    try {
      ...
      const response = await axios.get(`${base_url}/api/user/service/price/nid-card-order`);
  ...
  // Fetch orders for NID card service
  const fetchOrders = async () => {
    try {
      ...
      if (response.data.success) {
        // Filter orders by service name containing "এনআইডি কার্ড"
        const nidCardOrders = response.data.data.filter(order => 
          order.serviceName && order.serviceName.includes('এনআইডি কার্ড')
        );
        setOrders(nidCardOrders);
      }
    } catch (error) {
            ...
            <form onSubmit={handleSubmit}>
              ...
              {/* Price Information */}
                ...
                <p className={isDarkMode ? 'text-blue-300' : 'text-blue-700'}>
                  এই সেবার জন্য আপনার অ্যাকাউন্ট থেকে <span className="font-bold">{pricesLoading ? '...' : servicePrices.nidCard} টাকা</span> কাটা হবে। 
                </p>
              </div>
                        ...
                        <td colSpan="7" className="p-8 text-center">
                          <div className="flex flex-col items-center justify-center">
                            ...
                            <p className={`text-sm max-w-md transition-colors duration-300 ${
                              ...
                              {searchTerm ? 'আপনার সার্চের সাথে মিলছে এমন কোনো অর্ডার নেই' : 'আপনার প্রথম এনআইডি কার্ড অর্ডারটি এখনই করুন!'}
                            </p>
                            {searchTerm && (
                              <button
