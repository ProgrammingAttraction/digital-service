...
function IMEIToNumber() {
  ...
  const fetchNotice = async () => {
    try {
      ...
      const response = await axios.get(`${base_url}/api/user/service/notice/imei-to-number`);
      if (response.data) {
        setNotice(response.data.service);
      } else {
        setNotice('⚠️ নোটিশঃ আইএমইআই নাম্বার থেকে মোবাইল নাম্বার সার্ভিস। আইএমইআই নাম্বারটি অবশ্যই সঠিক দিতে হবে।');
      }
    } catch (error) {
  ...
  const fetchPrices = async () => {
    try {
      ...
      const response = await axios.get(`${base_url}/api/user/service/price/imei-to-number`);
      // Update service prices based on backend response
      const updatedPrices = {
        imeiToNumber: response.data.price
      };
  ...
  // Fetch orders for IMEI to Number service
  const fetchOrders = async () => {
    try {
      ...
      if (response.data.success) {
        // Filter orders by service name containing "আইএমইআই টু নাম্বার"
        const imeiOrders = response.data.data.filter(order => 
          order.serviceName && order.serviceName.includes('আইএমইআই টু নাম্বার')
        );
        setOrders(imeiOrders);
      }
    } catch (error) {
  ...
  // Validation function for IMEI numbers
  const validateForm = () => {
    const errors = {};
    
    if (!imei1.trim()) {
      errors.imei1 = 'আইএমইআই ১ নম্বর আবশ্যক';
    } else if (!/^\d{15}$/.test(imei1.trim())) {
      errors.imei1 = 'সঠিক ১৫ ডিজিটের আইএমইআই নম্বর লিখুন';
    }
    
    // IMEI 2 is optional, but if provided, validate it
    if (imei2.trim() && !/^\d{15}$/.test(imei2.trim())) {
      errors.imei2 = 'সঠিক ১৫ ডিজিটের আইএমইআই নম্বর লিখুন';
    }
  ...
  const handleSubmit = async (e) => {
    ...
    try {
      ...
      let dynamicData = `আইএমইআই ১ - ${imei1}`;
      if (imei2.trim()) {
        dynamicData += `\nআইএমইআই ২ - ${imei2}`;
      }
      if (extraData.trim()) {
        dynamicData += `\nঅতিরিক্ত তথ্য - ${extraData}`;
      }
      
      const fieldValues = {
        imei1: imei1.trim(),
        ...(imei2.trim() && { imei2: imei2.trim() }),
        ...(extraData.trim() && { extraData: extraData.trim() })
      };
      
      // Create request data object matching your backend expectations
      const requestData = {
        serviceId: 'imei_to_number',
        serviceName: 'আইএমইআই টু নাম্বার',
        serviceRate: servicePrices.imeiToNumber,
        serviceType: 'imei_to_number',
        orderType: 'text_file',
        quantity: 1,
        notes: dynamicData,
        fieldValues: fieldValues,
        totalAmount: servicePrices.imeiToNumber,
        urgency: 'normal'
      };
            ...
            <form onSubmit={handleSubmit}>
              <div className="space-y-4">
                ...
                <div>
                  ...
                  <input 
                    ...
                    placeholder="15 ডিজিটের আইএমইআই নম্বর" 
                  ...
                  />
                  {formErrors.imei1 && (
                    <p className="mt-1 text-xs text-red-500">{formErrors.imei1}</p>
                  )}
                  <p className={`mt-1 text-xs ${isDarkMode ? 'text-gray-400' : 'text-gray-500'}`}>
                    ১৫ ডিজিটের আইএমইআই নম্বর (যেমনঃ 358879052356214)
                  </p>
                </div>
                ...
                <div>
                  ...
                  <input 
                    ...
                    placeholder="15 ডিজিটের আইএমইআই নম্বর (যদি থাকে)" 
                    className={`w-full border rounded-md p-2 text-sm md:text-[15px] focus:outline-none focus:ring-1 h-[45px] placeholder:text-gray-400 transition-colors duration-300 ${
                      formErrors.imei2 
                        ? isDarkMode ? 'border-red-500 focus:ring-red-500' : 'border-red-400 focus:ring-red-400'
                        : isDarkMode 
                  ...
                  />
                  {formErrors.imei2 && (
                    <p className="mt-1 text-xs text-red-500">{formErrors.imei2}</p>
                  )}
                  <p className={`mt-1 text-xs ${isDarkMode ? 'text-gray-400' : 'text-gray-500'}`}>
                    ডুয়াল সিম ফোনের ক্ষেত্রে আইএমইআই ২ নম্বর (ঐচ্ছিক)
                  </p>
                </div>
                ...
                <div>
                  ...
                  <textarea 
                    ...
                    placeholder="ফোনের ব্র্যান্ড, মডেল বা অন্য কোন তথ্য লিখুন..." 
                    rows="3"
                  ...
                  />
                  <p className={`mt-1 text-xs ${isDarkMode ? 'text-gray-400' : 'text-gray-500'}`}>
                    ফোন সম্পর্কিত যেকোনো অতিরিক্ত তথ্য লিখতে পারেন
                  </p>
                </div>
              </div>
              ...
              <div className={`mt-4 mb-4 p-3 text-base border rounded-md flex justify-start items-center gap-2 transition-colors duration-300 ${
                ...
                <p className={isDarkMode ? 'text-blue-300' : 'text-blue-700'}>
                  এই সেবার জন্য আপনার অ্যাকাউন্ট থেকে <span className="font-bold">{pricesLoading ? '...' : servicePrices.imeiToNumber} টাকা</span> কাটা হবে। 
                </p>
              </div>
              {/* Submit Button */}
              <div className="mt-6">
                ...
                >
                  {loading ? (
                    ...
                    `অর্ডার করুন (${pricesLoading ? '...' : servicePrices.imeiToNumber} TK)`
                  )}
                </button>
              </div>
            </form>
          ...
          <div className={`mt-8 border rounded-lg shadow-sm p-4 md:p-6 transition-colors duration-300 ${
            ...
            <div className="flex justify-between items-center mb-6">
              <h2 className={`text-xl font-bold transition-colors duration-300 ${
                ...
                আইএমইআই টু নাম্বার অর্ডার ইতিহাস
              </h2>
                        ...
                        <td colSpan="7" className="p-8 text-center">
                          <div className="flex flex-col items-center justify-center">
                            ...
                            <p className={`text-sm max-w-md transition-colors duration-300 ${
                              ...
                              {searchTerm ? 'আপনার সার্চের সাথে মিলছে এমন কোনো অর্ডার নেই' : 'আপনার প্রথম আইএমইআই টু নাম্বার অর্ডারটি এখনই করুন!'}
                            </p>
                            {searchTerm && (
                              <button
