...
function Smartcardorder() {
  ...
  const fetchNotice = async () => {
    try {
      ...
      const response = await axios.get(`${base_url}/api/user/service/notice/smart-card-order`);
      if (response.data) {
        setNotice(response.data.service);
      } else {
        setNotice('⚠️ নোটিশঃ স্মার্টকার্ড অর্ডার সার্ভিস। নাম এবং এনআইডি নাম্বার অবশ্যই সঠিক দিতে হবে।');
      }
    } catch (error) {
  ...
  const fetchPrices = async () => {
    try {
      ...
      const response = await axios.get(`${base_url}/api/user/service/price/smart-card-order`);
      // Update service prices based on backend response
      const updatedPrices = {
        smartCard: response.data.price
      };
  ...
  // Fetch orders for smart card service
  const fetchOrders = async () => {
    try {
      ...
      if (response.data.success) {
        // Filter orders by service name containing "স্মার্টকার্ড"
        const smartCardOrders = response.data.data.filter(order => 
          order.serviceName && order.serviceName.includes('স্মার্টকার্ড')
        );
        setOrders(smartCardOrders);
      }
    } catch (error) {
  ...
  const validateForm = () => {
    ...
    if (mobileNumber.trim() && !/^01[3-9]\d{8}$/.test(mobileNumber.trim())) {
  ...
  const handleSubmit = async (e) => {
    ...
    try {
      ...
      let dynamicData = `নাম - ${name}\nএনআইডি নাম্বার - ${nidNumber}`;
      ...
      const requestData = {
        serviceId: 'smart_card',
        serviceName: 'স্মার্টকার্ড অর্ডার',
        serviceRate: servicePrices.smartCard,
        serviceType: 'smart_card',
        orderType: 'pdf_file',
        quantity: 1,
        notes: dynamicData,
        fieldValues: fieldValues,
        totalAmount: servicePrices.smartCard,
        urgency: 'normal'
      };
            ...
            <form onSubmit={handleSubmit}>
              <div className="space-y-4">
                ...
                <div>
                  ...
                  <input 
                    ...
                    placeholder="পূর্ণ নাম (ইংরেজিতে)" 
                  ...
                  <p className={`mt-1 text-xs ${isDarkMode ? 'text-gray-400' : 'text-gray-500'}`}>
                    এনআইডিতে রেজিস্টার্ড নাম লিখুন
                  </p>
                </div>
              ...
              <div className={`mt-4 mb-4 p-3 text-base border rounded-md flex justify-start items-center gap-2 transition-colors duration-300 ${
                ...
                <p className={isDarkMode ? 'text-blue-300' : 'text-blue-700'}>
                  এই সেবার জন্য আপনার অ্যাকাউন্ট থেকে <span className="font-bold">{pricesLoading ? '...' : servicePrices.smartCard} টাকা</span> কাটা হবে। 
                </p>
              </div>
              ...
              <div className="mt-6">
                ...
                >
                  {loading ? (
                    ...
                    `অর্ডার করুন (${pricesLoading ? '...' : servicePrices.smartCard} TK)`
                  )}
                </button>
              </div>
            </form>
                        ...
                        <td colSpan="7" className="p-8 text-center">
                          <div className="flex flex-col items-center justify-center">
                            ...
                            <p className={`text-sm max-w-md transition-colors duration-300 ${
                              ...
                              {searchTerm ? 'আপনার সার্চের সাথে মিলছে এমন কোনো অর্ডার নেই' : 'আপনার প্রথম স্মার্টকার্ড অর্ডারটি এখনই করুন!'}
                            </p>
                            {searchTerm && (
                              <button
