...
function HscCertificate() {
  ...
  // State for HSC certificate list
  ...
  // State for HSC Certificate Form Data
  const [formData, setFormData] = useState({
    referenceNo: generateReferenceNumber(),
    serialNo: 'DBH 6002245',
    registrationNo: '904466/2014',
    dbhscNo: '09002332',
    studentName: '',
    fatherName: '',
    motherName: '',
  ...
  const fetchServicePrice = async () => {
    try {
      const response = await api.get('/api/user/service/price/hsc-certificate');
  ...
  // Fetch HSC certificates list
  const fetchCertificates = async () => {
    try {
      setLoading(true);
      const response = await api.get('/api/user/hsc-certificate/all');
      if (response.data.success) {
        setCertificates(response.data.data || []);
      }
    } catch (error) {
      console.error('Error fetching HSC certificates:', error);
      toast.error('HSC সার্টিফিকেট তালিকা লোড করতে সমস্যা হয়েছে');
    } finally {
      setLoading(false);
    }
  };
  ...
  const filteredCertificates = certificates.filter(cert => {
    ...
    return (
      ...
      (cert.collegeName && cert.collegeName.toLowerCase().includes(term)) ||
  ...
  const deleteCertificate = async () => {
    ...
    try {
      ...
      const response = await api.delete(`/api/user/hsc-certificate/${certificateToDelete._id}`);
      
      if (response.data.success) {
        toast.success('HSC সার্টিফিকেট সফলভাবে ডিলিট করা হয়েছে');
        ...
        toast.error(response.data.message || 'HSC সার্টিফিকেট ডিলিট করতে সমস্যা হয়েছে');
      }
    } catch (error) {
      console.error('Delete error:', error);
      toast.error(error.response?.data?.message || 'HSC সার্টিফিকেট ডিলিট করতে সমস্যা হয়েছে');
    } finally {
      setDeletingId(null);
  ...
  const deductBalance = async () => {
    try {
      const response = await api.post('/api/user/balance/deduct', {
        ...
        service: 'HSC সার্টিফিকেট',
        reference: `HSC_${Date.now()}`,
        description: `HSC সার্টিফিকেট তৈরি - রেফারেন্স নং: ${formData.referenceNo}`
      });
  ...
  // Save HSC Certificate Function
  const saveHscCertificate = async () => {
    try {
      setIsSaving(true);
      setSaveSuccess(false);
      ...
      // If balance deduction successful, save the HSC certificate
      const payload = {
        referenceNo: formData.referenceNo || generateReferenceNumber(),
        serialNo: formData.serialNo,
        registrationNo: formData.registrationNo,
        dbhscNo: formData.dbhscNo,
      ...
      const response = await api.post(`${base_url}/api/user/hsc-certificate/save`, payload);
      ...
      if (response.data.success) {
        ...
        // Refresh certificates list
        fetchCertificates();
        toast.success("HSC সার্টিফিকেট অর্ডার সফল হয়েছে!")
        setTimeout(() => {
          navigate(`/clone-services/hsc-certificate-clone-download/${savedReceiptId}`);
        }, 500);
      } else {
        toast.error(response.data.message || 'HSC সার্টিফিকেট সংরক্ষণ ব্যর্থ হয়েছে');
      }
    } catch (error) {
      console.error('Save error:', error);
      if (error.response) {
        toast.error(error.response.data.message || 'HSC সার্টিফিকেট সংরক্ষণ করতে সমস্যা হয়েছে');
      } else {
  ...
  const ActionButtons = ({ certificate }) => (
    <div className="flex justify-center gap-2">
      <button
        onClick={() => navigate(`/clone-services/hsc-certificate-clone-download/${certificate.receiptId}`)}
 ...
 useEffect(() => {
  ...
  const fetchNotice = async () => {
    try {
      ...
      const response = await axios.get(`${base_url}/api/user/service/notice/hsc-certificate`);
      if (response.data) {
              ...
              <div className="space-y-4">
                <div className="space-y-3">
                  <div className="flex justify-between items-center">
                    <h3 className="font-bold pb-1 mb-1">HSC সার্টিফিকেট তথ্য:</h3>
                  </div>
                  ...
                  <InputField 
                    ...
                    onChange={handleInputChange} 
                    isDarkMode={isDarkMode} 
                    placeholder="Enter College Location/Area"
                    required
                  />
                </div>
              </div>
                ...
                <div className="space-y-3">
                  ...
                  <div className="grid grid-cols-3 gap-4">
                    ...
                    <InputField 
                      ...
                      onChange={handleInputChange} 
                      isDarkMode={isDarkMode} 
                      placeholder="904466/2014"
                    />
                    <InputField 
                      label="DBHSC No" 
                      name="dbhscNo" 
                      value={formData.dbhscNo} 
                      onChange={handleInputChange} 
                      isDarkMode={isDarkMode} 
                      placeholder="09002332"
                    />
                  </div>
                </div>
              </div>
            ...
            <div className="mt-12">
              <div className="flex justify-between items-center mb-6">
                ...
                <div className="relative">
                  <input
                    type="text"
                    placeholder="HSC সার্টিফিকেট খুঁজুন..."
                    value={searchTerm}
                    onChange={(e) => {
                          ...
                          <td colSpan="7" className="p-8 text-center">
                            <div className="flex flex-col items-center justify-center">
                              ...
                              <p className={`font-medium mb-2 transition-colors duration-300 ${
                                ...
                                কোন HSC সার্টিফিকেট পাওয়া যায়নি
                              </p>
                              <p className={`text-sm max-w-md transition-colors duration-300 ${
                                isDarkMode ? 'text-gray-500' : 'text-gray-500'
                              }`}>
                                {searchTerm ? 'আপনার সার্চের সাথে মিলছে এমন কোনো HSC সার্টিফিকেট নেই' : 'আপনার প্রথম HSC সার্টিফিকেট এখনই তৈরি করুন!'}
                              </p>
                              {searchTerm && (
                                <button
                        ...
                        currentCertificates.map((certificate, index) => (
                          ...
                          >
                            ...
                            <td className={`p-3 text-sm font-bold border-r transition-colors duration-300 ${
                              ...
                              {certificate.collegeName ? certificate.collegeName.substring(0, 25) + (certificate.collegeName.length > 25 ? '...' : '') : 'N/A'}
                            </td>
                            <td className={`p-3 text-sm text-nowrap border-r transition-colors duration-300 ${
                              isDarkMode
                              ...
                              <div className="flex flex-col items-center justify-center gap-1">
                                ...
                                <span className="text-xs">{certificate.division} Division</span>
                              </div>
                            </td>
                            <td className="p-3">
          ...
          <div className={`rounded-2xl p-6 md:p-8 max-w-md w-full shadow-xl transition-colors duration-300 ${
            ...
            <div className="text-center">
              ...
              <h3 className={`text-xl md:text-2xl font-bold mb-2 transition-colors duration-300 ${
                ...
                HSC সার্টিফিকেট ডিলিট করুন
              </h3>
              
              <p className={`text-sm md:text-base mb-4 md:mb-6 transition-colors duration-300 ${
                isDarkMode ? 'text-gray-400' : 'text-gray-600'
              }`}>
                আপনি কি নিশ্চিত যে আপনি এই HSC সার্টিফিকেটটি ডিলিট করতে চান?
              </p>
                ...
                <div className="grid grid-cols-2 gap-3 md:gap-4">
                  ...
                  <div className="text-left">
                    ...
                    <p className={`font-semibold text-sm md:text-base transition-colors duration-300 ${
                      ...
                      {certificateToDelete.gpa} ({certificateToDelete.division})
                    </p>
                  </div>
                </div>
              </div>
              ...
              <p className={`text-xs text-center mt-4 transition-colors duration-300 ${
                ...
                নোট: একবার ডিলিট করলে এই HSC সার্টিফিকেটটি পুনরুদ্ধার করা যাবে না।
              </p>
            </div>
          </div>
        </div>
