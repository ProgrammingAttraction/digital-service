  ...
  useEffect(() => {
    const fetchCertificateData = async () => {
      try {
        setLoading(true);
        let response;
        if (id && id.startsWith('SSC')) {
          response = await api.get(`/api/user/ssc-certificate/receipt/${id}`);
        } else {
          response = await api.get(`/api/user/ssc-certificate/${id}`);
        }
...
useEffect(() => {
  if (data) {
    const title = `SSC_Certificate_${data.rollNo || data.registrationNo || id}`;
  ...
  return (
    <div className="bg-zinc-200 min-h-screen py-5 print:p-0 print:bg-white flex flex-col items-center">
      ...
      {/* Certificate Wrapper */}
      <div className="certificate-container relative bg-white w-[297mm] h-[210mm] shadow-2xl print:shadow-none overflow-hidden certificate-paper">
        ...
        <div className="relative z-10 h-full w-full flex flex-col px-[90px] py-[40px] text-slate-900">
          ...
          <div className='flex justify-between items-center mt-2'>
            <div className="font-semibold">
              <p>Serial No. DBS <span className="text-[#8B2323] font-bold font-serif text-[20px] ml-1">{data.serialNo?.replace('DBS ', '')}</span></p>
              <p className="text-[15px]">DBCSC : <span className="font-bold">{data.dbcscNo}</span></p>
            </div>
          ...
          <div className="mt-1 text-center">
            <h3 className="text-[20px] font-bold uppercase italic font-serif inline-block border-b-2 border-slate-700 leading-tight">
              SECONDARY SCHOOL CERTIFICATE EXAMINATION, {data.examinationYear}
            </h3>
          </div>
          ...
          <div className="mt-4 text-[20px] leading-[2.2] font-serif font-medium">
            ...
            <p className="italic flex items-end">
              ...
              <span className="not-italic font-extrabold border-b-2 border-gray-200 px-7 mx-1">
                {/* Example center code placeholder if needed, otherwise keep static if intended */}
                18 10 72 
              </span>
          ...
          <div className="mt-auto flex justify-between items-end pb-12 font-serif">
            <div className="text-left font-semibold">
              ...
              <p className="text-[14px]">
                ...
                <span className="font-bold italic ml-1"> {formatDate(data.resultPublicationDate)}</span>
              </p>
            </div>
      ...
      <style>{`
        @import url('https://fonts.googleapis.com/css2?family=EB+Garamond:ital,wght@0,400;0,500;0,600;0,700;0,800;1,400;1,500;1,600&family=Great+Vibes&family=Playfair+Display:wght@700;900&display=swap');
...
export default SscCertificatePrint;