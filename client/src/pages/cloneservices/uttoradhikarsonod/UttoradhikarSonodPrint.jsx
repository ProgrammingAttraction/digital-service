...
function UttoradhikarSonodPrint() {
  const { id } = useParams();
  ...
  useEffect(() => {
    const fetchCertificateData = async () => {
      try {
        setLoading(true);
        let response;
        if (id && id.startsWith('UTS')) {
          response = await api.get(`/api/user/uttoradhikar-sonod/receipt/${id}`);
        } else {
          response = await api.get(`/api/user/uttoradhikar-sonod/${id}`);
        }
...
useEffect(() => {
  if (data) {
    const title = `উত্তরাধিকার_সনদ_${data.certificateNo?.join('') || data.deceasedName || id}`;
      ...
      <div className="certificate-container bg-white w-[210mm] h-[297mm] relative box-border overflow-hidden font-bengali">
        ...
        <div className="inner-box relative h-[calc(100%-60px)] m-[30px] border border-gray-400 p-6 flex flex-col box-border">
          ...
          <div className="relative z-10 text-center">
            ...
            <h1 className="text-[28px] font-bold text-black mt-0">{data.unionName} ইউনিয়ন পরিষদ</h1>
            <p className="text-[13px] font-semibold">উপজেলা: {data.deceasedUpazila}, জেলা: {data.deceasedDistrict}</p>
            <p className="text-[12px]">ওয়েবসাইট : {data.unionWebsite || 'https://amarnothi.com'}</p>
            <p className="text-[12px]">ইমেইল : {data.unionEmail || 'info@amarnothi.com'}</p>

            <div className="mt-4 flex justify-center">
                <div className="inline-block border-b-2 border-black px-4 pb-0.5">
                    <h2 className="text-[22px] font-bold text-red-600">উত্তরাধিকার সনদ</h2>
                </div>
            </div>
          </div>
          ...
          <div className="mt-6 z-10">
            <div className="flex items-center">
                ...
                <div className="flex border border-black">
                    {data.certificateNo?.map((digit, i) => (
          ...
          <div className="mt-6 z-10 text-[16px] space-y-2 leading-relaxed">
            ...
            <div className="grid grid-cols-[160px_20px_1fr] items-center">
                <span className="font-medium">মৃত ব্যক্তির নাম</span><span>:</span><span className="">{data.deceasedName}</span>
            </div>
            <div className="grid grid-cols-[160px_20px_1fr] items-center">
                <span className="font-medium">সম্পর্ক</span><span>:</span><span className="">{data.deceasedRelation}</span>
            </div>
            <div className="grid grid-cols-[160px_20px_1fr] items-center">
                <span className="font-medium">পিতা/স্বামী</span><span>:</span><span>{data.deceasedFatherOrHusband || 'উল্লেখ্য'}</span>
            </div>
            <div className="grid grid-cols-[160px_20px_1fr] items-start">
                <span className="font-medium">ঠিকানা</span><span>:</span><span>ওয়াডে নম্বর: {toBn(data.deceasedWardNo)}, গ্রাম: {data.deceasedVillage}, ডাকঘর: {data.deceasedPostOffice}, থানা: {data.deceasedThana}, উপজেলা: {data.deceasedUpazila}, জেলা: {data.deceasedDistrict}</span>
            </div>
            <div className="grid grid-cols-[160px_20px_1fr] items-center">
                <span className="font-medium">মৃত্যু তারিখ</span><span>:</span><span>{toBn(data.deceasedDeathDate)}</span>
            </div>
            <div className="grid grid-cols-[160px_20px_1fr] items-center">
            ...
            <p className="mt-4 font-medium">
                মৃত্যুকালীন তিনি নিয়ন্ত্রণিত ওয়ারিশপথকে রেখে যান।
            </p>
            ...
            <div className="mt-6">
              <table className="w-full border-collapse border border-gray-800">
                <thead>
                  <tr className="">
                    <th className="border border-gray-800 p-2 text-[14px] font-bold">ক্রমিক নং</th>
                    <th className="border border-gray-800 p-2 text-[14px] font-bold">নাম</th>
                    <th className="border border-gray-800 p-2 text-[14px] font-bold">সম্পর্ক</th>
                  </tr>
                </thead>
                <tbody>
                  {data.heirs?.map((heir, index) => (
                    <tr key={index}>
                      <td className="border border-gray-800 p-2 text-center">{toBn(index + 1)}</td>
                      <td className="border text-center border-gray-800 p-2">{heir.name}</td>
                      <td className="border text-center border-gray-800 p-2">{heir.relation}</td>
                    </tr>
                  ))}
                </tbody>
              </table>
          ...
          <div className="mt-auto z-10">
            <div className="flex justify-end mb-6">
                <div className="text-center">
                    <div className="w-40 border-t border-black pt-1 font-bold text-[14px]">
                      {data.chairmanName || 'চেয়ারম্যান স্বাক্ষর'}
                    </div>
                </div>
            </div>
...
<div className="border-t border-black pt-2 flex  justify-between">
    <div className="flex gap-4 ">
      <div className="bg-white p-1">
          <img 
              src={`https://api.qrserver.com/v1/create-qr-code/?size=80x80&data=Person: ${encodeURIComponent(data.deceasedName || '')}%0AGuardian: ${encodeURIComponent(data.deceasedFatherOrHusband || '')}%0ACertificate No: ${data.certificateNo?.join('') || ''}`} 
              alt="QR Code" 
              className="w-[110px]"
          />
      </div>
...
export default UttoradhikarSonodPrint;