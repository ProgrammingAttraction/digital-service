...
function Nidmake2() {
  ...
  const fetchServicePrice = async () => {
    try {
      ...
      const response = await axios.get(
        `${base_url}/api/user/service/price/nid-make2`,
        {
          headers: {
  ...
  const fetchOrders = async () => {
    try {
      ...
      const response = await axios.get(
        `${base_url}/api/user/nid-make2-orders`,
        {
          headers: {
            'Authorization': `Bearer ${token}`,
            'userid': userId
      ...
      if (response.data.success) {
        ...
        toast.error(response.data.message || 'এনআইডি অর্ডার তালিকা লোড করতে সমস্যা হয়েছে');
      }
    } catch (error) {
      console.error('Error fetching NID orders:', error);
      toast.error('এনআইডি অর্ডার তালিকা লোড করতে সমস্যা হয়েছে');
    } finally {
  ...
  const extractDataFromPDF = async (pdfFile) => {
    ...
    try {
      ...
      const response = await axios.post(
        `${base_url}/api/user/sign-to-nid2`,
        formData,
        {
          headers: {
  ...
  // Handle form submission - UPDATED FOR BOTH FILE AND URL
  const handleSubmit = async (e) => {
    ...
    try {
      ...
      // Call backend API to create NID order
      const response = await axios.post(
        `${base_url}/api/user/create-nid-make2-order`, 
        submitFormData, 
        {
          headers: {
            'Authorization': `Bearer ${token}`,
      ...
      if (response.data.success) {
        toast.success("এনআইডি অর্ডার সফল হয়েছে!");
        ...
        navigate(`/auto-services/new-nid2-download/${response.data.data?.receiptId}`);
        
        // Show success message with order details
        toast.success(`অর্ডার নং: ${response.data.data?.receiptId || 'N/A'}`);
      } else {
        setError(response.data.message || "Failed to save NID data");
        toast.error(response.data.message || 'এনআইডি অর্ডার সংরক্ষণ ব্যর্থ হয়েছে');
      }
    } catch (err) {
  ...
  const deleteOrder = async () => {
    ...
    try {
      ...
      const response = await axios.delete(
        `${base_url}/api/user/nid-make2-order/${orderToDelete._id}`,
        {
          headers: {
            'Authorization': `Bearer ${token}`,
            'userid': userId
      ...
      if (response.data.success) {
        toast.success('এনআইডি অর্ডার সফলভাবে ডিলিট করা হয়েছে');
        // Refresh orders list
        fetchOrders();
        // Update user balance in localStorage
        ...
        toast.error(response.data.message || 'এনআইডি অর্ডার ডিলিট করতে সমস্যা হয়েছে');
      }
    } catch (error) {
      console.error('Delete error:', error);
      toast.error(error.response?.data?.message || 'এনআইডি অর্ডার ডিলিট করতে সমস্যা হয়েছে');
    } finally {
      setDeletingId(null);
  ...
  const ActionButtons = ({ order }) => (
    <div className="flex justify-center gap-2">
      <button
        onClick={() => navigate(`/auto-services/new-nid2-download/${order.receiptId}`)}
 ...
 useEffect(() => {
  ...
  const fetchNotice = async () => {
    try {
      ...
      const response = await axios.get(`${base_url}/api/user/service/notice/nid-make2`);
      if (response.data) {
      ...
      <div className={`font-anek lg:ml-72 mt-[9vh] min-h-screen transition-colors duration-300 ${isDarkMode ? 'bg-gray-900 text-gray-100' : 'bg-gray-50 text-gray-700'}`}>
        ...
        <main className="min-h-[93vh] p-3 md:p-6">
          ...
          <div className="flex items-center gap-2 mb-6">
            <h1 className={`text-xl md:text-2xl font-bold ${isDarkMode ? 'text-theme_color' : 'text-theme_color'}`}>এনআইডি মেক 2</h1>
          </div>
            ...
            <div className="mt-12">
              <div className="flex justify-between items-center mb-6">
                ...
                <div className="relative">
                  <input
                    type="text"
                    placeholder="এনআইডি অর্ডার খুঁজুন..."
                    value={searchTerm}
                    onChange={(e) => {
                          ...
                          <td colSpan="9" className="p-8 text-center">
                            <div className="flex flex-col items-center justify-center">
                              ...
                              <p className={`font-medium mb-2 transition-colors duration-300 ${
                                ...
                                কোন এনআইডি অর্ডার পাওয়া যায়নি
                              </p>
                              <p className={`text-sm max-w-md transition-colors duration-300 ${
                                isDarkMode ? 'text-gray-500' : 'text-gray-500'
                              }`}>
                                {searchTerm ? 'আপনার সার্চের সাথে মিলছে এমন কোনো এনআইডি অর্ডার নেই' : 'আপনার প্রথম এনআইডি অর্ডার এখনই তৈরি করুন!'}
                              </p>
                              {searchTerm && (
                                <button
    ...
    <div className={`rounded-2xl p-5 md:p-6 max-w-sm w-full shadow-xl transition-colors duration-300 ${
      ...
      <div className="text-center">
        ...
        <h3 className={`text-lg md:text-xl font-bold mb-1.5 transition-colors duration-300 ${
          ...
          এনআইডি অর্ডার ডিলিট করুন
        </h3>
        
        <p className={`text-xs md:text-sm mb-3 md:mb-4 transition-colors duration-300 ${
          isDarkMode ? 'text-gray-400' : 'text-gray-600'
        }`}>
          আপনি কি নিশ্চিত যে আপনি এই এনআইডি অর্ডারটি ডিলিট করতে চান?
        </p>
