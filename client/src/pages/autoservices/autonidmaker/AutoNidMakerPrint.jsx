...
function AutoNidMakerPrint() {
  ...
  useEffect(() => {
    const fetchNIDOrderData = async () => {
      try {
        setLoading(true);
        // Changed API endpoint to use auto-nid-maker
        const response = await axios.get(`${base_url}/api/user/auto-nid-maker/order/${id}`, {
          headers: {
        ...
        if (response.data.success) {
          ...
          setError(response.data.message || 'Auto NID order not found');
        }
      } catch (err) {
        console.error('Error fetching auto NID order:', err);
        setError('Failed to load Auto NID order data');
      } finally {
        setLoading(false);
      }
    };
            ...
            <div className="flex pt-[2px] px-[2px] relative z-10">
              <div className="w-[75px] flex flex-col items-center">
                <div className=" bg-white overflow-hidden">
                  <img src={orderData.nidPhoto || orderData.photo} className="w-[75px] h-[84px] object-cover" alt="User" />
                </div>
              ...
              <div className="flex-1 pl-2 text-[13px] pt-1">
                <div className="flex mb-1 items-baseline">
                  ...
                  <span className="font-bold text-gray-800 text-[12px]">{orderData.nameBangla}</span>
                </div>
                ...
                <div className="flex items-baseline text-[12px]">
                  ...
                  <span className="text-red-500 text-[12px] font-bold">{orderData.nationalId}</span>
                </div>
              </div>
            </div>
          </div>
            ...
            <div className="pt-2 h-full flex flex-col">
              ...
              <div className="flex justify-between relative items-center text-[10px] text-gray-800 mt-2 border-b-[0.5px] border-gray-800 px-1">
                <div>
                  ...
                  <span className="text-red-600 font-bold ml-1">{orderData.bloodGroup || 'N/A'}</span>
                </div>
...
export default AutoNidMakerPrint;