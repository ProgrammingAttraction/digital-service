...
function Autotincertificateprint() {
  const { id } = useParams();
    ...
    const fetchCertificateData = async () => {
      try {
        ...
        if (id && id.startsWith('TIN')) {
          response = await api.get(`/api/user/aut-tin-certificate-api/receipt/${id}`);
        } else {
...
useEffect(() => {
  if (data) {
    const title = `TIN_Certificate_${id}`;
...
export default Autotincertificateprint;