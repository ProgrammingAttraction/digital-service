...
function PoliceClearance() {
  ...
  // State for Police Clearance Form Data
  ...
  useEffect(() => {
    const fetchServicePrice = async () => {
      try {
        const response = await axios.get(`${base_url}/api/user/service/price/police-clearance`);
  ...
  const fetchReceipts = async () => {
    try {
      setLoading(true);
      const response = await api.get('/api/user/police-clearance/all');
      if (response.data.success) {
        setReceipts(response.data.data || []);
      }
    } catch (error) {
      console.error('Error fetching receipts:', error);
      toast.error('পুলিশ ক্লিয়ারেন্স তালিকা লোড করতে সমস্যা হয়েছে');
    } finally {
      setLoading(false);
    }
  };
  ...
  const filteredReceipts = receipts.filter(receipt => {
    ...
    return (
      ...
      (receipt.passportNo && receipt.passportNo.toLowerCase().includes(term)) ||
      (receipt.referenceNo && receipt.referenceNo.toLowerCase().includes(term)) ||
      (receipt.policeStation && receipt.policeStation.toLowerCase().includes(term)) ||
  ...
  const deleteReceipt = async () => {
    ...
    try {
      ...
      const response = await api.delete(`/api/user/police-clearance/${receiptToDelete._id}`);
      
      if (response.data.success) {
        toast.success('পুলিশ ক্লিয়ারেন্স সফলভাবে ডিলিট করা হয়েছে');
        ...
        toast.error(response.data.message || 'পুলিশ ক্লিয়ারেন্স ডিলিট করতে সমস্যা হয়েছে');
      }
    } catch (error) {
      console.error('Delete error:', error);
      toast.error(error.response?.data?.message || 'পুলিশ ক্লিয়ারেন্স ডিলিট করতে সমস্যা হয়েছে');
    } finally {
      setDeletingId(null);
  ...
  const deductBalance = async () => {
    try {
      const response = await api.post('/api/user/balance/deduct', {
        amount: servicePrice,
        service: 'পুলিশ ক্লিয়ারেন্স সার্টিফিকেট',
        reference: `POL_${Date.now()}`,
        description: `পুলিশ ক্লিয়ারেন্স সার্টিফিকেট তৈরি - রেফারেন্স নং: ${formData.referenceNo}`
      });
  ...
  // Save Police Clearance Function
  const savePoliceClearance = async () => {
    try {
      setIsSaving(true);
      setSaveSuccess(false);

      // Validate required fields
      const requiredFields = ['policeStation', 'passportNo', 'fatherName', 'issueDate', 
      ...
      if (!dateRegex.test(formData.issueDate)) {
        toast.error('সঠিক তারিখ ফরম্যাট দিন (YYYY-MM-DD)');
        setIsSaving(false);
        return;
      }
      ...
      // If balance deduction successful, save the police clearance
      const payload = {
        ...
        po: formData.po || 'P/O',
        psUpozila: formData.psUpozila || 'P/S Upozila',
      ...
      const response = await api.post(`${base_url}/api/user/police-clearance/save`, payload);
      ...
      if (response.data.success) {
        ...
        // Refresh receipts list
        fetchReceipts();
        toast.success("পুলিশ ক্লিয়ারেন্স অর্ডার সফল হয়েছে!")
        setTimeout(() => {
          navigate(`/clone-services/police-clearance-clone-download/${savedReceiptId}`);
        }, 500);
      } else {
        toast.error(response.data.message || 'পুলিশ ক্লিয়ারেন্স সংরক্ষণ ব্যর্থ হয়েছে');
      }
    } catch (error) {
      console.error('Save error:', error);
      if (error.response) {
        toast.error(error.response.data.message || 'পুলিশ ক্লিয়ারেন্স সংরক্ষণ করতে সমস্যা হয়েছে');
      } else {
  ...
  const ActionButtons = ({ receipt }) => (
    <div className="flex justify-center gap-2">
      <button
        onClick={() => navigate(`/clone-services/police-clearance-clone-download/${receipt.receiptId}`)}
  ...
  const handleSuccessContinue = () => {
    if (orderSuccessData?.orderId) {
      navigate(`/clone-services/police-clearance-clone-download/${orderSuccessData.orderId}`);
    }
    closeSuccessPopup();
  };
// -------------------------------notice-funtion-----------------------
 ...
 useEffect(() => {
  ...
  const fetchNotice = async () => {
    try {
      ...
      const response = await axios.get(`${base_url}/api/user/service/notice/police-clearance`);
      if (response.data) {
      ...
      <div className={`font-anek lg:ml-72 mt-[9vh] min-h-screen transition-colors duration-300 ${isDarkMode ? 'bg-gray-900 text-gray-100' : 'bg-gray-50 text-gray-700'}`}>
        ...
        <main className="min-h-[91vh] p-4 md:p-8">
          <div className="flex justify-between items-center mb-6">
            <h1 className={`text-lg md:text-2xl font-bold ${isDarkMode ? 'theme_color' : 'text-theme_color'}`}>
              ...
              <div className="space-y-4">
                <div className="space-y-3">
                  <div className="flex justify-between items-center">
                    <h3 className="font-bold pb-1 mb-1">পুলিশ ক্লিয়ারেন্স তথ্য:</h3>
                  </div>
              ...
              <div className="space-y-4">
                <div className="space-y-3">
                  ...
                  <InputField 
                    label="P/S Upozila *" 
                    name="psUpozila" 
                    value={formData.psUpozila} 
                    onChange={handleInputChange} 
                    isDarkMode={isDarkMode} 
                    placeholder="P/S Upozila"
                  />
                  
                  {/* Date Input Field - Allows both typing and calendar */}
                  <div className="flex flex-col flex-1">
                    ...
                    <p className={`text-xs mt-1 ${isDarkMode ? 'text-gray-500' : 'text-gray-500'}`}>
                      তারিখ টাইপ করুন (YYYY-MM-DD) ফরম্যাটে অথবা ক্যালেন্ডার আইকনে ক্লিক করুন
                    </p>
                  </div>
            ...
            <div className="mt-12">
              <div className="flex justify-between items-center mb-6">
                ...
                <div className="relative">
                  <input
                    type="text"
                    placeholder="পুলিশ ক্লিয়ারেন্স খুঁজুন..."
                    value={searchTerm}
                    onChange={(e) => {
                          ...
                          <td colSpan="7" className="p-8 text-center">
                            <div className="flex flex-col items-center justify-center">
                              ...
                              <p className={`font-medium mb-2 transition-colors duration-300 ${
                                ...
                                কোন পুলিশ ক্লিয়ারেন্স পাওয়া যায়নি
                              </p>
                              <p className={`text-sm max-w-md transition-colors duration-300 ${
                                isDarkMode ? 'text-gray-500' : 'text-gray-500'
                              }`}>
                                {searchTerm ? 'আপনার সার্চের সাথে মিলছে এমন কোনো পুলিশ ক্লিয়ারেন্স নেই' : 'আপনার প্রথম পুলিশ ক্লিয়ারেন্স এখনই তৈরি করুন!'}
                              </p>
                              {searchTerm && (
                                <button
          ...
          <div className={`rounded-2xl p-6 md:p-8 max-w-md w-full shadow-xl transition-colors duration-300 ${
            ...
            <div className="text-center">
              ...
              <p className={`text-sm md:text-base mb-4 md:mb-6 transition-colors duration-300 ${
                ...
                আপনার পুলিশ ক্লিয়ারেন্স সার্টিফিকেট অর্ডারটি সফলভাবে প্লেস করা হয়েছে।
              </p>
          ...
          <div className={`rounded-2xl p-6 md:p-8 max-w-md w-full shadow-xl transition-colors duration-300 ${
            ...
            <div className="text-center">
              ...
              <h3 className={`text-xl md:text-2xl font-bold mb-2 transition-colors duration-300 ${
                ...
                পুলিশ ক্লিয়ারেন্স ডিলিট করুন
              </h3>
              
              <p className={`text-sm md:text-base mb-4 md:mb-6 transition-colors duration-300 ${
                isDarkMode ? 'text-gray-400' : 'text-gray-600'
              }`}>
                আপনি কি নিশ্চিত যে আপনি এই পুলিশ ক্লিয়ারেন্সটি ডিলিট করতে চান?
              </p>
              ...
              <p className={`text-xs text-center mt-4 transition-colors duration-300 ${
                ...
                নোট: একবার ডিলিট করলে এই পুলিশ ক্লিয়ারেন্সটি পুনরুদ্ধার করা যাবে না।
              </p>
            </div>
          </div>
        </div>
