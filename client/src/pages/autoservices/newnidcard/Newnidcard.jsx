...
function Newnidcard() {
  ...
  const fetchServicePrice = async () => {
    try {
      ...
      const response = await axios.get(
        `${base_url}/api/user/service/price/nid-make`,
        {
          headers: {
  ...
  const handleSubmit = async (e) => {
    ...
    try {
      ...
      const response = await axios.post(
        `${base_url}/api/user/create-nid-order`, 
        submitFormData, 
        {
          headers: {
            'Authorization': `Bearer ${token}`,
      ...
      if (response.data.success) {
        ...
        navigate(`/auto-services/new-nid-download/${response.data.data?.receiptId}`);
  ...
  const deleteOrder = async () => {
    ...
    try {
      ...
      const response = await axios.delete(
        `${base_url}/api/user/nid-orders/${orderToDelete._id}`,
        {
          headers: {
            'Authorization': `Bearer ${token}`,
            'userid': userId
  ...
  const ActionButtons = ({ order }) => (
    <div className="flex justify-center gap-2">
      <button
        onClick={() => navigate(`/auto-services/new-nid-download/${order.receiptId}`)}
 ...
 useEffect(() => {
  ...
  const fetchNotice = async () => {
    try {
      ...
      const response = await axios.get(`${base_url}/api/user/service/notice/nid-make`);
      if (response.data) {
      ...
      <div className={`font-anek lg:ml-72 mt-[9vh] min-h-screen transition-colors duration-300 ${isDarkMode ? 'bg-gray-900 text-gray-100' : 'bg-gray-50 text-gray-700'}`}>
        ...
        <main className="min-h-[93vh] p-3 md:p-6">
          ...
          <div className="flex items-center gap-2 mb-6">
            <h1 className={`text-xl md:text-2xl font-bold ${isDarkMode ? 'text-theme_color' : 'text-theme_color'}`}>এনআইডি মেক</h1>
          </div>
