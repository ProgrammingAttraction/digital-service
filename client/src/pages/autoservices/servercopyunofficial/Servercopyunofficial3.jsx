...
function Servercopyunofficial3() {
  const { id } = useParams();
  ...
  useEffect(() => {
    const fetchCertificateData = async () => {
      try {
        ...
        // Fix: Based on your JSON, the relevant fields are in response.data.data 
        // OR directly in response.data. Adjusting to catch both possibilities.
        if (response.data.success || response.data.apiSuccess) {
...
useEffect(() => {
  ...
  // Helper to handle both string addresses and object addresses
  const formatAddress = (addr) => {
    if (typeof addr === 'string') return addr;
    if (addr && addr.addressLine) return addr.addressLine;
    return '';
  };
        ...
        <div className="w-[82%] mx-auto mt-2 bg-[#BBE6ED]">
          <table className="w-full border-collapse">
            <tbody>
              ...
              <DataRow label="ফরম নম্বর" value={data.oldNid} />
              ...
              <DataRow label="লিঙ্গ" value={data.gender?.toLowerCase() === 'male' ? 'পুরুষ' : 'মহিলা'} />
              ...
              <tr className="border border-gray-100">
                <td colSpan="2" className="px-2 py-1.5 text-[13px] leading-tight">
                  {formatAddress(data.currentAddress || data.presentAddress)}
                </td>
              </tr>
              ...
              <tr className="border border-gray-100 ">
                <td colSpan="2" className="px-2 py-1.5 text-[13px] leading-tight">
                  {formatAddress(data.permanentAddressLine || data.permanentAddress)}
                </td>
              </tr>
            </tbody>
          </table>
...
export default Servercopyunofficial3;