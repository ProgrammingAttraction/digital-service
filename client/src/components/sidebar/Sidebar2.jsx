...
import { BiIdCard, BiSearchAlt } from 'react-icons/bi';
...
const Sidebar = ({ isOpen, closeSidebar }) => {
  ...
  const adminWhatsAppNumber = '8801853825711'; // Replace with actual admin WhatsApp number
  const adminWhatsAppMessage = 'Hello%20Admin,%20I%20need%20assistance%20with%20my%20account.'; // URL encoded message
  ...
  useEffect(() => {
    const fetchSocialMedia = async () => {
      try {
        ...
        // Set fallback social media links in case of error
        setSocialMediaLinks([
          {
            name: 'WhatsApp',
            icon: <FaWhatsapp />,
  ...
  // Menu Groups Data with Admin Support added
  const menuGroups = [
    {
      items: [
    ...
    //   title: 'অর্ডার ',
    //   items: [
    //     { icon: <FaFileAlt />, text: 'PDF অর্ডার', url: '/order/file-order', id: 'file-order' },
    //     { icon: <FaFileWord />, text: 'Text অর্ডার', url: '/order/text-order', id: 'text-order' },
    //   ]
    // },
    {
