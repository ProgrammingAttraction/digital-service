...
function Allorders() {
  ...
  const fetchOrders = useCallback(async () => {
    try {
      ...
      const response = await axios.get(`${base_url}/api/sub-admin/orders`, {
        headers: {
    ...
    <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-4 gap-4 mb-6">
      <div className="bg-white rounded-lg shadow p-4">
        ...
        <div className="mt-2 text-xs text-gray-500">
          <span className="text-green-600">Completed: {statistics?.completedOrders || 0}</span>
          {' | '}
          <span className="text-yellow-600">Pending: {statistics?.pendingOrders || 0}</span>
        </div>
      </div>
      <div className="bg-white rounded-lg shadow p-4">
        <div className="flex items-center justify-between">
        ...
        <div className="mt-2 text-xs text-gray-500">
          Avg. Order: ৳{statistics?.totalOrders ? Math.round(statistics.totalAmount / statistics.totalOrders) : 0}
        </div>
      </div>
      <div className="bg-white rounded-lg shadow p-4">
        <div className="flex items-center justify-between">
    ...
    <div className="font-anek text-gray-700 lg:ml-72 mt-[9vh]">
      ...
      <main className="min-h-screen bg-gray-50 p-4 md:p-6">
        <div className="w-full mx-auto">
          {/* Page Header */}
          <div className="mb-6">
            <h1 className="text-2xl font-bold text-theme_color2 mb-1">All Orders</h1>
            <p className="text-gray-600 mt-1">Manage all customer orders and track their status</p>
          </div>
