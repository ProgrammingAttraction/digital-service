...
// Modern Toggle Switch Component (Added)
...
function Activeusers() {
  ...
  const [updatingStatus, setUpdatingStatus] = useState(null); // Track which user is being updated (Added)
  ...
  const fetchActiveUsers = useCallback(async () => {
    try {
      ...
      const response = await axios.get(`${base_url}/api/sub-admin/active-users`, {
        headers: {
      ...
      toast.error(error.response?.data?.error || 'Failed to fetch active users');
    } finally {
      setLoading(false);
    }
  ...
  // Handle user status update (Added)
  const handleStatusUpdate = async (userId, newStatus) => {
    try {
      ...
      const response = await axios.put(
        `${base_url}/api/sub-admin/users/${userId}/status`,
        { status: newStatus },
        {
          headers: {
            'Authorization': `Bearer ${token}`,
      ...
      if (response.data.success) {
        ...
        // If user is now inactive, remove from current view (since this is active users page)
  ...
  const handleDeleteUser = async () => {
    try {
      ...
      const response = await axios.delete(`${base_url}/api/sub-admin/users/${selectedUser._id}`, {
        headers: {
          'Authorization': `Bearer ${token}`,
          'Content-Type': 'application/json'
        }
  ...
  const handleDeleteAllUsers = async () => {
    ...
    try {
      ...
      const response = await axios.delete(`${base_url}/api/sub-admin/users/delete-all`, {
        headers: {
          'Authorization': `Bearer ${token}`,
          'Content-Type': 'application/json'
        },
  ...
  const handleDeleteAllFilteredUsers = async () => {
    try {
      ...
      const response = await axios.delete(`${base_url}/api/sub-admin/users/delete-all-filtered`, {
        headers: {
          'Authorization': `Bearer ${token}`,
          'Content-Type': 'application/json'
        },
    ...
    <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-4 gap-4 mb-6">
      <div className="bg-white rounded-lg shadow p-4">
        ...
        <div className="mt-2 text-xs text-gray-500">
          <span className="text-green-600">All users are active</span>
        </div>
      </div>
    ...
    <div className="font-anek text-gray-700 lg:ml-72 mt-[9vh]">
      ...
      <main className="min-h-screen bg-gray-50 p-4 md:p-6">
        <div className="w-full mx-auto">
          {/* Page Header */}
          <div className="mb-6">
            <h1 className="text-2xl font-bold text-green-600 mb-1">Active Users</h1>
            <p className="text-gray-600 mt-1">Manage all active users and their accounts</p>
          </div>
            ...
            <div className="overflow-x-auto">
              {loading ? (
                <div className="text-center py-12">
                  ...
                  <h3 className="text-lg font-medium text-gray-700">Loading active users...</h3>
                </div>
              ) : users.length === 0 ? (
                <div className="text-center py-12">
                  <User className="w-12 h-12 text-gray-400 mx-auto mb-4" />
                  <h3 className="text-lg font-medium text-gray-700">No active users found</h3>
                  <p className="text-gray-500 mt-1">Try adjusting your search or filters</p>
                  <button
                    onClick={clearAllFilters}
                    className="mt-4 px-4 py-2 bg-green-600 text-white rounded-lg cursor-pointer transition-colors duration-200"
                  >
                    Clear All Filters
                  </button>
                </div>
                ...
                <table className="w-full">
                  <thead className="bg-gray-50">
                    <tr>
                      ...
                      <th className="px-6 py-3 text-left text-sm font-medium text-gray-500 uppercase tracking-wider">
                        Deactivate {/* Changed from Actions */}
                      </th>
                      <th className="px-6 py-3 text-left text-sm font-medium text-gray-500 uppercase tracking-wider">
                        Actions
                      </th>
                  ...
                  <tbody className="bg-white divide-y divide-gray-200">
                    {users.map((user) => (
                      <tr key={user._id} className="hover:bg-gray-50 transition-colors duration-150">
                        ...
                        <td className="px-6 py-4 md:text-[15px] whitespace-nowrap text-sm text-gray-500">
                          {formatDate(user.createdAt)}
                        </td>
                        <td className="px-6 py-4 whitespace-nowrap">
                          {/* Added ModernToggleSwitch */}
                          <ModernToggleSwitch
                ...
                <div className="flex items-center justify-between">
                  <div className="hidden md:flex items-center gap-4">
                    <div className="text-sm text-gray-700">
                      ...
                      of <span className="font-medium">{totalItems}</span> active users
                    </div>
                    ...
                    {totalItems > itemsPerPage && (
                      <button
                        ...
                        className="text-sm text-green-600 hover:text-green-800 transition-colors duration-150 cursor-pointer"
                      >
                  ...
                  <div className="flex justify-between  w-full md:w-auto items-center space-x-2">
                    <select
                      ...
                      className="px-3 py-1 border border-gray-300 rounded-lg text-sm focus:outline-none focus:ring-2 focus:ring-green-500 cursor-pointer"
                    >
